...
//
   ...
   for(var i=0;i<parseInt(peopleCount);i++)
   {
    ...
    //  strhtml+="<td ><input name='InRoomCountry' id='country"+i+"' type='text' value='中国' desc='客人国籍' class='input80 validate[required]' /></td>";
...
var DBAmount=$("#DBAmount").val();
   ...
   if(DBAmount!=null&&DBAmount!=''&&DBAmount!='0'&&parseInt(peopleCount)>=parseInt(DBAmount)){
...
var roomtypeid='<ww:property value="roomid"/>';
...
var ret="-1@-1@-1@-1";
 	...
 	$.ajax({
	         type:"POST",
	         url:"hoteluserbook!valadateElongHotelIsBook.action",
	         data:{startDate:stime,endDate:endtime,EarliestArrivalTime:EarliestArrivalTime,LatestArrivalTime:LatestArrivalTime,s_hotelid:hotelid,pricecode:RatePlanId,roomid:roomtypeid,NumberOfRooms:NumberOfRooms,s_price:TotalPrice,para:Math.floor(Math.random()*100)},
	...
	if(rets[0]!='-1'){
		if(rets[0]=='0'&&rets[1]!='0'){
			alert("预订该酒店需要担保金额:"+rets[1]+"元,担保后如未入住将扣除担保费用!!!");
			//return;
			
			ShowXinYongKaDiv();
			//document.getElementById("book").s
			 //$("#book").show();
			  $("#submitreg").show();
			 $("#book").show();
		}else{
...
function CheckCardNO(cardno){
	...
	$.ajax({
	         type:"POST",
	         url:"hoteluserbook!valadateElongHotelCardNO.action",
	         data:{cardno:cardno,para:Math.floor(Math.random()*100)},
	         //beforeSend:function(){$("#HotelRoomPrice").html("<img src='images/loadding.gif' />")},
	         async:false,              
	         success:function(data){
	       		// alert(data);
...
<table width="100%" border="0" class="box" cellspacing="0"
    ...
    <!-- 以下是验证担保的条件 -->
    <input name="DBstartDate" type="hidden" id="DBstartDate" value="<ww:property value="listGuaranteeRule.get(0).StartDate"/>" />
    <input name="DBendDate" type="hidden" id="DBendDate" value="<ww:property value="listGuaranteeRule.get(0).EndDate"/>" />
    <input name="DBWeekSet" type="hidden" id="DBWeekSet" value="<ww:property value="listGuaranteeRule.get(0).WeekSet"/>" />
	<input name="DBStartTime" type="hidden" id="DBStartTime" value="<ww:property value="listGuaranteeRule.get(0).StartTime"/>" />
	<input name="DBEndTime" type="hidden" id="DBEndTime" value="<ww:property value="listGuaranteeRule.get(0).EndTime"/>" />
	<input name="DBAmount" type="hidden" id="DBAmount" value="<ww:property value="listGuaranteeRule.get(0).Amount"/>" />
	</tr>
</table>
...
<!--<tr>
		...
		<ww:property value="listGuaranteeRule.get(0).StartTime"/>
...
<!-- 信用卡信息开始 -->
<div id="xinyongka">
<div class="kongfang" style="color: red"><b class="lan_14c">信用卡信息：</b>*表示必填，如您填写信息有误，可能导致下单失败，视为无效订单。担保订单无未入住将扣除担保费用!!!</div>
<table width="100%" border="0" class="box" cellspacing="0"
	cellpadding="0" bordercolor="#cccccc"
	style="border-bottom: none; line-height: 24px; border-top: 2px solid #779BCA">
	<tr>
	...
	<tr>
		...
		<input name="cardno" desc="信用卡卡号" type="text" id="kahao" class="input100 validate[required]" onchange="CheckCardNO(this.value)" value="" />
		
		<span style="color: red;">*</span></td>
	
		<td width="90" class="box_yin" style="display: none" align="center" id="cvvtext">信用卡CVV</td>
		<td class="box_wu" style="display: none" id="cvvtd">
		<input name="cardcvv" id="cvv" type="text" value=""  desc="信用卡CVV" class="input100"/>
		<span style="color: red;">*</span></td>
	</tr>
...
<tr>
		...
		<td class="box_wu" width="240">
		<!--
		<input name="cardyear" style="width: 40px;"  type="text" id="nian" maxlength="4"   class="input100"  value="" />
		-->
		...
		年
		<!--
		<input name="cardmonth" style="width: 40px;" maxlength="2"  type="text" id="yue" class="input100 "  value="" />
		-->
...
<div style="width: 98%; text-align: center; height: 80px;">
<input type="button" class="button_up" onclick="javascript:window.history.back()" value="上一步" style="margin-top:3px;" />
<input name="submitnext" type="button" value="验证订单" onclick="valadateIsBook();" id="submitreg" class="button_next">
<input type="submit" id="book" style="display: none" class="button_next" value="提交订单" onclick="checkform();" />
</div>
