        ...
        items: [
		     
		    {
				...
				handler : function(item){
					window.location.href="informationinfo!toadd.action?<ww:property value="url"/>";
			...
			,{  
				...
				handler : function(item){
						document.form1.action="informationinfo!toedit.action?id="+selectId;
            ...
            },
			{
				...
				handler : function(item){
					Ext.MessageBox.show({title:'删除',width:300, msg:'确认删除吗？',buttons: Ext.MessageBox.YESNO,fn:function(btn){
						...
						if(btn=='yes')
						{
						document.form1.action="informationinfo!delete.action?id="+selectId;
						document.form1.submit();
			...
			'-',
			{
				...
				handler :function(item){
						document.form1.action="informationinfo!tocheck.action?id="+selectId;
						document.form1.submit();
...
Ext.onReady(function(){
  ...
  var comboxWithTree = new Ext.form.ComboBox({  
	...
	var tree = new Ext.tree.TreePanel({ 
		...
		comboxWithTree.setValue("<ww:property value="getInformationName(s_type)"/>");
...
<table width="100%" border="0" cellpadding="0" cellspacing="0"
	...
	<tr>
		<td width="100%" height="29" class="box-bottom bg"><b
			class="anse"><span class="font-blue-cu">&nbsp;&nbsp;&nbsp;资讯中心详细信息列表</span></b></td>
	</tr>
	<tr>
		<td valign="top">
		<form name="form1" method="post" action="informationinfo.action">
           ...
           <table width="1000" border="0" align="center" cellpadding="0" cellspacing="0">
              <tr>
                ...
                <td width="130" height="20" align="right">类型：</td>    <td><span style="HEIGHT: 71px">
                 <!-- <input id="startnum2"   style="WIDTH: 181px" name="s_type" value="<ww:property value="s_type"/>" /> --> 
                  <div id='comboxWithTree'></div>
					<input type="hidden" id="parentid" name="s_type"
						value='<ww:property value="s_type"/>'/></td>
                </span></td>
									...
									<tbody>
										...
										<ww:iterator value="listInformationinfo">
											<tr id="<ww:property value="id"/>" align="center"
												...
												<ww:if test="info.length()>30">	<ww:property value="SubString(info,27)" />....</ww:if>
												<ww:else>
												<ww:property value="info" />
												</ww:else>
												</td>
												<td class="table_color"><ww:property
													value="getInformationName(typeid)" /></td>
												<td class="table_color"><ww:property
													value="formatTimestamp(createtime)" /></td>
												<!--<td class="table_color"><ww:property value="memberid" /></td>  -->
												<td class="table_color"><ww:if test="state==1">启用</ww:if>
												<ww:if test="state==0">禁用</ww:if></td>
...
<script language="JavaScript">
	function toadd(){
		window.location="informationinfo!toadd.action?<ww:property value="url"/>";
	}
	function deleteItem(){
			...
			if(length== undefined){
				...
				if(document.form1.selectid.checked ==true)
				{
						...
						if(temp==true){
							...
							document.form1.action="informationinfo!delete.action?id="+document.form1.selectid.value;
							document.form1.submit();
						}
						return;
				}
			  	 ...
			  	 if ( length == null || length==0 ){
					     ...
					     for (var i = 0; i < length; i++)
					      ...
					      if(len==1){
					      	var temp = confirm('确认删除吗？');
							if(temp==true){
								document.form1.action="informationinfo!delete.action?id="+uvalue;
								document.form1.submit();
							}
							return;
					      }else if (len>1){
					      	var temp = confirm('确认删除吗？');
					      	if(temp==true){
					      		document.form1.action="informationinfo!batch.action?opt=1";
								document.form1.submit();
							}
							return;
 ...
 function updateItem(){
			...
			if(length== undefined){
				...
				if(document.form1.selectid.checked ==true)
				{
						...
						document.form1.action="informationinfo!toedit.action?id="+document.form1.selectid.value;
						document.form1.submit();
			  	 ...
			  	 if ( length == null || length==0 ){
					     ...
					     for (var i = 0; i < length; i++)
					      ...
					      if(len==1){
					      		document.form1.action="informationinfo!toedit.action?id="+uvalue;
								document.form1.submit();
								return;
					      }else if (len>1){
 ...
 function checkItem(){
			...
			if(length== undefined){
				...
				if(document.form1.selectid.checked ==true)
				{
						...
						document.form1.action="informationinfo!tocheck.action?id="+document.form1.selectid.value;
						document.form1.submit();
			  	 ...
			  	 if ( length == null || length==0 ){
					     ...
					     for (var i = 0; i < length; i++)
					      ...
					      if(len==1){
					      		document.form1.action="informationinfo!tocheck.action?id="+uvalue;
								document.form1.submit();
								return;
					      }else if (len>1){
					      	var temp = confirm('只能选择一项进行操作？');
