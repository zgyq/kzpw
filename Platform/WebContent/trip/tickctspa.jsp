        ...
        items: [
		     
		    {
				...
				handler : function(item){
					window.location.href="tickctspa!toadd.action?<ww:property value="url"/>";
			...
			,{  
				...
				handler : function(item){
						document.form1.action="tickctspa!toedit.action?id="+selectId;
            ...
            },
			{
				...
				handler : function(item){
					Ext.MessageBox.show({title:'删除',width:300, msg:'确认删除吗？',buttons: Ext.MessageBox.YESNO,fn:function(btn){
						...
						if(btn=='yes')
						{
						document.form1.action="tickctspa!delete.action?id="+selectId;
						document.form1.submit();
...
<table width="100%" border="0" cellpadding="0" cellspacing="0"
	...
	<tr>
		<td width="100%" height="29" class="box-bottom bg"><b
			class="anse"><span class="font-blue-cu">&nbsp;&nbsp;&nbsp;票务温泉列表</span></b></td>
	</tr>
	<tr>
		<td valign="top">
		<form name="form1" method="post" action="tickctspa.action">
							...
							<tr>
								...
								<table id="menutable" width="99%" border="1" align="center"
									...
									<tbody>
										...
										<ww:iterator value="listTickctspa">
											<tr id="<ww:property value="id"/>" align="center"
												...
												<td class="table_color"><span style="cursor: pointer;" title="<ww:property value="name"/>"><ww:property value="subStringstr(name,25)" /></span></td>
												<td class="table_color"><ww:property value="homeprice" /></td>
												<td class="table_color"><ww:property value="price" /></td>
												<td class="table_color"><ww:property value="period" /></td>
												<td class="table_color"><ww:property value="desc" /></td>
...
<script language="JavaScript">
	function toadd(){
		window.location="tickctspa!toadd.action?<ww:property value="url"/>";
	}
	function deleteItem(){
			...
			if(length== undefined){
				...
				if(document.form1.selectid.checked ==true)
				{
						...
						if(temp==true){
							...
							document.form1.action="tickctspa!delete.action?id="+document.form1.selectid.value;
							document.form1.submit();
						}
						return;
				}
			  	 ...
			  	 if ( length == null || length==0 ){
					     ...
					     for (var i = 0; i < length; i++)
					      ...
					      if(len==1){
					      	var temp = confirm('确认删除吗？');
							if(temp==true){
								document.form1.action="tickctspa!delete.action?id="+uvalue;
								document.form1.submit();
							}
							return;
					      }else if (len>1){
					      	var temp = confirm('确认删除吗？');
					      	if(temp==true){
					      		document.form1.action="tickctspa!batch.action?opt=1";
								document.form1.submit();
							}
							return;
 ...
 function updateItem(){
			...
			if(length== undefined){
				...
				if(document.form1.selectid.checked ==true)
				{
						...
						document.form1.action="tickctspa!toedit.action?id="+document.form1.selectid.value;
						document.form1.submit();
			  	 ...
			  	 if ( length == null || length==0 ){
					     ...
					     for (var i = 0; i < length; i++)
					      ...
					      if(len==1){
					      		document.form1.action="tickctspa!toedit.action?id="+uvalue;
								document.form1.submit();
								return;
					      }else if (len>1){
 ...
 function checkItem(){
			...
			if(length== undefined){
				...
				if(document.form1.selectid.checked ==true)
				{
						...
						document.form1.action="tickctspa!tocheck.action?id="+document.form1.selectid.value;
						document.form1.submit();
			  	 ...
			  	 if ( length == null || length==0 ){
					     ...
					     for (var i = 0; i < length; i++)
					      ...
					      if(len==1){
					      		document.form1.action="tickctspa!tocheck.action?id="+uvalue;
								document.form1.submit();
								return;
					      }else if (len>1){
					      	var temp = confirm('只能选择一项进行操作？');
