...
<form name="form1" method="post" action="hotelnoauditingorder.action"  >
...
<script language="JavaScript">
	...
	function hotel_show_info(id) {
		var res = true ;
		$.ajax({
			url: 'hotelnoauditingorder!grab.action?h_orderId=' + id + '&time=' + new Date().getTime(),
  			async: false ,
  			dataType : 'json' ,
  			success : function(result) {
  				if(result.flag == 'failure') {
  					res = false ;
  					window.alert("已经被处理!") ;
  				} else if(result.flag == 'beyond') {
  					res = false ;
  					window.alert("已经有个" + result.grabnohandlenum + "未处理的订单,不许在处理新的订单") ;
  				}
  			}
		}) ;
		if(!res) {
		...
		document.forms.form1.action="hotelnoauditingorder!tocheck.action?id=" + id + "&" + '<ww:property value="url"/>' ;
