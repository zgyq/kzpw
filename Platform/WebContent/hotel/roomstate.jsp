        ...
        items: [
		     
		    {
				...
				handler : function(item){
					window.location.href="roomstate!tabs.action?hotelId=<ww:property value="hotelId"/>&tabtype=addroomstate&<ww:property value="url"/>";
			...
			,{  
				...
				handler : function(item){
						document.roomstateform.action="roomstate!tabs.action?hotelId=<ww:property value="hotelId"/>&tabtype=editroomstate&id="+selectId;
						document.roomstateform.submit();
				}
            }
					...
					<tr>
						...
						<!--<table width="99%" border="0" cellspacing="0" cellpadding="0">
							...
							<tr>
								...
								<div align="right"><a
									href="roomstate!tabs.action?hotelId=<ww:property value="hotelId"/>&tabtype=addroomstate&<ww:property value="url"/>">新增</a>
								&nbsp;&nbsp;&nbsp;<a href="#" onclick="updateItem()"><strong class="font-red">修改</strong></a> &nbsp;&nbsp;&nbsp;<a
									...
									onclick="checkItem()"><strong class="font-red">审核</a>
									</div>
								</td>
							</tr>
						</table>
						...
						<table width="100%" height="40" border="0" cellspacing="0" cellpadding="0" class="lj" style="font-size:12px; font-weight:bold; line-height:25px;">
				           <tr>
				               ...
				               <td width="75%" align="right"><a href="roomstate!tabs.action?hotelId=<ww:property value="hotelId"/>&tabtype=addroomstate&<ww:property value="url"/>"><div class="button_h font-red"  >新增</div></a></td>
							...
							<tbody>
								...
								<ww:iterator value="listRoomstate">
									<tr id="<ww:property value="id"/>" align="center"
										...
										<td><ww:if test="state==2"><a href="roomstate!cancel.action?hotelId=<ww:property value="hotelId"/>&id=<ww:property value="id"/>">取消满房</a></ww:if></td>
									</tr>
								</ww:iterator>
			...
			if(length== undefined){
				...
				if(document.roomstateform.selectid.checked ==true)
				{
						...
						if(temp==true){
							...
							document.roomstateform.action="roomstate!delete.action?id="+document.roomstateform.selectid.value +"&hotelId=<ww:property value="hotelId"/>";
							document.roomstateform.submit();
						}
						return;
				}
			  	 ...
			  	 if ( length == null || length==0 ){
					     ...
					     for (var i = 0; i < length; i++)
					      ...
					      if(len==1){
					      	var temp = confirm('确认删除吗？');
							if(temp==true){
								document.roomstateform.action="roomstate!delete.action?id="+uvalue+"&hotelId=<ww:property value="hotelId"/>";
								document.roomstateform.submit();
							}
							return;
					      }else if (len>1){
					      	var temp = confirm('确认删除吗？');
					      	if(temp==true){
					      		document.roomstateform.action="roomstate!batch.action?opt=1&hotelid=<ww:property value="hotelId"/>";
								document.roomstateform.submit();
							}
							return;
 ...
 function updateItem(){
			...
			if(length== undefined){
				...
				if(document.roomstateform.selectid.checked ==true)
				{
						...
						document.roomstateform.action="roomstate!tabs.action?id="+ document.roomstateform.selectid.value+"&hotelId=<ww:property value="hotelId"/>"+ "&tabtype=editroomstate";
						document.roomstateform.submit();
			  	 ...
			  	 if ( length == null || length==0 ){
					     ...
					     for (var i = 0; i < length; i++)
					      ...
					      if(len==1){
					      		document.roomstateform.action="roomstate!tabs.action?id="+uvalue+ "&hotelId=<ww:property value="hotelId"/>"+ "&tabtype=editroomstate";
								document.roomstateform.submit();
								return;
					      }else if (len>1){
