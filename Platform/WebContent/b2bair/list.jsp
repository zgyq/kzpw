	 ...
	 //数据验证
        function checkdata() {
            ...
            if ($("#txtStartCity").val() == "" || $("#hidStarCityCode").val() == "") {
                ...
                loadingoverlay("<img src='main_cx/img/loading.gif' /><br/><span style='color:#fff'>正在为您查询航班信息，请等待...</span>");
                return true;
            }
            return false;
        }
  ...
  function lodprice(hb,cw,ct,tm,index){
	...
	function lodZrateByNfd(scode,ecode,stime,aircode,flight,cabincode,price,index){
			//alert(scode+","+ecode+","+stime+","+aircode+","+flight+","+cabincode+","+price+","+index);
		 	 ...
		 	 //鼠标滑过弹出层
		function xianshi(id){
			document.getElementById("tuiDiv"+id).style.display="block";                
 		} 
 		function yincang(id){
			document.getElementById("tuiDiv"+id).style.display="none";            
 		}
...
<div class="right">
     <div class="box">
                 ...
                 <!--  搜索框 begin-->
                 <form name="form1" method="post" action="b2bair!toTicketList.action" id="form1">
                 <div class="search2" style="background:#f5f5f5; padding:8px; margin:50px 0">
                         <table  cellspacing="0" cellpadding="1" align="center" style="width:100%">
                                    
                                    <tr>
                                         <td>
                                             航程类型：
                                              <select name="s_traveltype">
                                              <option value="1">单程</option>
                                              <option value="2" selected="selected">往返</option>
                                              <lect>
                                         </td>
                                         
                                         <td> 出发站：<input type="text" style="width:50%" name="s_depcityname"  id="txtStartCity" value="<ww:property value="s_depcityname" />"
											...
											<input type="hidden" name="s_depcitycode" id="hidStarCityCode" value="<ww:property value="s_depcitycode" />">
											
											
											</td>
                                         <td> 到达站：<input type="text" style="width:50%" id="txtEndCity" name="s_arrcityname"
                                        onfocus="popUp('txtEndCity','hidEndCityCode')"
										onclick="popUp('txtEndCity','hidEndCityCode')"
										onblur="showSearch('txtEndCity',1)" autocomplete="off" value="<ww:property value="s_arrcityname" />"
										onkeyup="popup_hide();suggest.display('txtEndCity','hidEndCityCode',event);" />
										<input type="hidden" value="<ww:property value="s_arrcitycode" />" name="s_arrcitycode" id="hidEndCityCode">
										</td>
                                         <td> 出发日期：<input type="text" style="width:40%" name="s_startdate" id="txtFromDate" onclick="WdatePicker({minDate:'%y-%M-{%d}'})" /></td>
                                         <td> 返程日期：<input type="text" style="width:40%" name="s_backdate" id="txtBackDate" onclick="WdatePicker({minDate:'%y-%M-{%d+1}'})" /></td>
                                        <td> <a href="#" onclick="" class="btn3"><img src="b2bair/img/search3.png" width="50" height="24" /></a></td>
                 ...
                 <!--日期 begin-->
                  <div class="date">
                     <table  cellspacing="0" cellpadding="1" align="center" style="width:100%">
                        ...
                        <td <ww:if test="train.startdate==GetDate(s_startdate,#state.index-(Calculate(s_startdate)>3?3:Calculate(s_startdate)))">class="td-now"</ww:if> ><a href="#" onclick="CheckDay('<ww:property value="GetDate(s_startdate,#state.index-(Calculate(s_startdate)>3?3:Calculate(s_startdate)))" />');return false;"><span>
                        <ww:property value="GetDate(s_startdate,#state.index-(Calculate(s_startdate)>3?3:Calculate(s_startdate)))" />
                        </span>
                        <ww:property value="getWeekStr(GetDate(s_startdate,#state.index-(Calculate(s_startdate)>3?3:Calculate(s_startdate))))" />
                        	</a></td>
                         </ww:iterator> 
                         </tr>
                     </table>
                           ...
                           <div class="show-con">
                               ...
                               <!-- 所有舱位展示 begin-->
                                     <table id='divallcabin_<ww:property value="#state.index" />'  cellspacing="0" cellpadding="1" align="center" class="all-space" style="display: none;">
                                         ...
                                         <tr class="tr-con">
                                            <td class="long-td"><ww:property value="Discount/10" />折<ww:property value="GetCabinType(Discount,Special,cabin)" />(<ww:property value="cabin" />)<b><a href="#" onmouseover="xianshi(id='<ww:property value="#cabState.index"/>');" onmouseout="yincang(id='<ww:property value="#cabState.index"/>');">退改签</a></b><div id="tuiDiv<ww:property value="#cabState.index"/>"   class="refund" style="width:300px; height:120px; border:1px dotted #dfdfdf; font-size:12px; background:#e2e2e2; padding:10px; z-index:999; position:absolute; left:160px; border:1px solid #fd492a; color:#fd492a;display:none;"><ww:property
							value="CabinRules" /></div></td>
