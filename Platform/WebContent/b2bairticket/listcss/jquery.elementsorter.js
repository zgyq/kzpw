(function($) {
	$.fn.extend({
		...
		sort : function(params,rows) {
		alert(params);
			// expected params
			/*
			 * { sortOn: '.selector', //selector of the column to sort on
			 * direction: 'asc' | 'desc' //optional..defaults to asc sortType:
			 * 'string | date | number' //optional..defaults to string }
			 * 
			 * OR
			 * 
			 * an array of the above obj for multi sorting. The order in the
			 * array determines order of precedence
			 */
			...
			var sortFunc = function(a, b) {
				...
				for (var i = 0; i < params.length; i++) {
					...
					retval = innerSort(typedData.a, typedData.b,
							params[i].direction);
					if (retval != 0)
						break;
				}
...
//				var rows = $("tbody[id*='divlowprice_']");
