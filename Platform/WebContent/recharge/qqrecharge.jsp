...
function ajaxnextqmoneyRecharge(id,onumber){
   ...
   $.ajax({
      url:"ofcard!ajaxnextqmoneyRecharge.action",
      type:"POST",
      data:{rechid:id},
      beforeSend:function(){
		...
		<td valign="top">
         ...
         <table width="100%" border="1" cellspacing="0" cellpadding="0"  class="table_color">
          ...
          <tr>
            ...
            <ww:if test="getLoginsessionagent().agenttype==1">
            <select name="s_state" onchange="UpdateState('qq','<ww:property value="ordernumber"/>',this.value);">
								<option value="11" <ww:if test="rechstate==11">selected</ww:if> >等待支付</option>
								<option value="0" <ww:if test="rechstate==0">selected</ww:if> >充值中</option>
								<option value="1" <ww:if test="rechstate==1">selected</ww:if> >充值成功</option>
								<option value="9" <ww:if test="rechstate==9">selected</ww:if> >充值失败</option>
		    ...
		    <td class="table_color">
							    ...
							    <ww:if test="paystate==1"><span style="color: red">已支付</span></ww:if>
							   </td>
		   <td class="table_color">
		    ...
		    <input class="button108" type="button" onclick="window.location.href='ofcard!nextQmoneyRechargePay.action?rechid=<ww:property value="id"/>'"  id='nextpaybtn<ww:property value="id"/>' type="button" value="立即支付"/>
		     ...
		     <ww:if test="getLoginsessionagent().agenttype==1">
			    <ww:if test="paystate==1&&rechstate!=1&&rechstate!=0">
