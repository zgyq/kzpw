...
<table width="100%" border="0" cellpadding="0" cellspacing="0" align="center" class="box">
  <tr>
    <td width="100%" height="29" class="box-bottom bg"><b class="anse"><span class="font-blue-cu">&nbsp;&nbsp;&nbsp;给:<strong><ww:property value="getmembername(customeruser.id)"/></strong>分配角色</span></b></td>
  </tr>
  <tr>
    <td  valign="top">
	...
	<ww:iterator value="listSystemrole" id="listSysright">
	      <tr align="center"
		   ...
		   <td>
		   <input type="checkbox" id="selectid" onclick="allocat(this,<ww:property value="customeruser.id"/>);" name="selectid" value="<ww:property value="id"/>" 
		   <ww:if test="hasRole(customeruser.id,id)">checked</ww:if>/>
		   </td>
 ...
 <tr><td align="center"><input type="button" onclick="fanhui();" class="button_d font-white" value="返&nbsp;&nbsp;回"/> </td></tr>
...
function allocat(select,memid)
	...
	//alert(memid+"00"+select);
		...
		{
			window.location.href='customeruser!enable.action?roid='+select.value+'&memberid=<ww:property value="customeruser.id"/>';
		
		}else{
		
			window.location.href='customeruser!unable.action?roid='+select.value+'&memberid=<ww:property value="customeruser.id"/>';
