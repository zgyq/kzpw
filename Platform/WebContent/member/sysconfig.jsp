        ...
        items: [
		     
		    {
				...
				handler : function(item){
					window.location.href="sysconfig!toadd.action?<ww:property value="url"/>";
			...
			,{  
				...
				handler : function(item){
						document.form1.action="sysconfig!toedit.action?id="+selectId;
            ...
            },
			{
				...
				handler : function(item){
					Ext.MessageBox.show({title:'删除',width:300, msg:'确认删除吗？',buttons: Ext.MessageBox.YESNO,fn:function(btn){
						...
						if(btn=='yes')
						{
						document.form1.action="sysconfig!delete.action?id="+selectId;
						document.form1.submit();
			...
			'-',
			{
				...
				handler :function(item){
						document.form1.action="sysconfig!tocheck.action?id="+selectId;
						document.form1.submit();
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" align="center" class="box">
  <tr>
    <td width="100%" height="29" class="box-bottom bg"><b class="anse"><span class="font-blue-cu">&nbsp;&nbsp;&nbsp;系统配置表列表</span></b></td>
  </tr>
  <tr>
    <td  valign="top">
<form name="form1" method="post" action="sysconfig.action">
            ...
            <td width="100%"><table id="menutable" width="99%" border="1" align="center" class="table_color">
              <tbody>
                <tr class="tbody_color">
                  ...
                  <th class="table_color">ID</th><th class="table_color">名称</th><th class="table_color">值</th><th class="table_color">描述</th>
                
        
			</tr>

		<ww:iterator value="listSysconfig">
...
<td  class="table_color"><ww:property value="id"/></td><td  class="table_color"><ww:property value="name"/></td><td  class="table_color"><ww:property value="value"/></td><td  class="table_color"><ww:property value="description"/></td>
...
<script language="JavaScript">
	function toadd(){
		window.location="sysconfig!toadd.action?<ww:property value="url"/>";
	}
	function deleteItem(){
			...
			if(length== undefined){
				...
				if(document.form1.selectid.checked ==true)
				{
						...
						if(temp==true){
							...
							document.form1.action="sysconfig!delete.action?id="+document.form1.selectid.value;
							document.form1.submit();
						}
						return;
				}
			  	 ...
			  	 if ( length == null || length==0 ){
					     ...
					     for (var i = 0; i < length; i++)
					      ...
					      if(len==1){
					      	var temp = confirm('确认删除吗？');
							if(temp==true){
								document.form1.action="sysconfig!delete.action?id="+uvalue;
								document.form1.submit();
							}
							return;
					      }else if (len>1){
					      	var temp = confirm('确认删除吗？');
					      	if(temp==true){
					      		document.form1.action="sysconfig!batch.action?opt=1";
								document.form1.submit();
							}
							return;
 ...
 function updateItem(){
			...
			if(length== undefined){
				...
				if(document.form1.selectid.checked ==true)
				{
						...
						document.form1.action="sysconfig!toedit.action?id="+document.form1.selectid.value;
						document.form1.submit();
			  	 ...
			  	 if ( length == null || length==0 ){
					     ...
					     for (var i = 0; i < length; i++)
					      ...
					      if(len==1){
					      		document.form1.action="sysconfig!toedit.action?id="+uvalue;
								document.form1.submit();
								return;
					      }else if (len>1){
 ...
 function checkItem(){
			...
			if(length== undefined){
				...
				if(document.form1.selectid.checked ==true)
				{
						...
						document.form1.action="sysconfig!tocheck.action?id="+document.form1.selectid.value;
						document.form1.submit();
			  	 ...
			  	 if ( length == null || length==0 ){
					     ...
					     for (var i = 0; i < length; i++)
					      ...
					      if(len==1){
					      		document.form1.action="sysconfig!tocheck.action?id="+uvalue;
								document.form1.submit();
								return;
					      }else if (len>1){
					      	var temp = confirm('只能选择一项进行操作？');
