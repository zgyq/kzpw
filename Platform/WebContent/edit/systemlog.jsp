...
'id','code','address','name','systemuser','createtime','userid'
	...
	new Ext.Button( 
 	         { 
	         	...
	         	text: '新建',
		        handler: function(){
		           //window.location.href='systemlog!toadd.action';
		        if(gwin){gwin.close();}
		        gwin = Ext.create('Ext.Window', {
			        ...
			        items: [{
			            border: false,
			            html:'<iframe   src="systemlog!toadd.action?<ww:property value="url"/>" width="100%" height="100%" frameborder="0" ></iframe>'
						...
						if(btn=='yes')
						{
						 var selectStr =""; 
				            if(sm.selected.items.length>0){
				            	...
				            	//window.location.href="systemlog!batch.action?opt=1&"+selectStr;
				            	Ext.Ajax.request({
							       url: "systemlog!batch.action?opt=1&"+selectStr,
        ...
        items: [
		     
		    {
				...
				handler : function(item){
					//window.location.href="systemlog!toadd.action?<ww:property value="url"/>";
					 if(gwin){gwin.close();}
		        gwin= Ext.create('Ext.Window', {
            ...
            
              {
				...
				handler : function(item){
					//window.location.href="systemlog!toedit.action?id="+selectId+"&<ww:property value="url"/>";
					//alert(selectId);
					 if(gwin){gwin.close();}
		        gwin= Ext.create('Ext.Window', {
			        ...
			        items: [{
			            border: false,
			            html:'<iframe   src="systemlog!toedit.action?id='+selectId+'&<ww:property value="url"/>" width="100%" height="100%" frameborder="0" ></iframe>'
				...
				handler : function(item){
					Ext.MessageBox.show({title:'删除',width:300, msg:'确认删除吗？',buttons: Ext.MessageBox.YESNO,fn:function(btn){
						...
						if(btn=='yes')
						{
						//	window.location.href="systemlog!delete.action?id="+selectId;
							Ext.Ajax.request({
							       url: "systemlog!delete.action?id="+selectId,
							       success: function(response){
							        //var text = response.responseText;
							           store.load();
							        // process server response here
...
<form action="systemlog.action">
<table width="100%" border="0" cellpadding="0" cellspacing="0" >
  <tr>
    <td valign="top" background="../images/titleL3_bg.gif"> <table width="100%"  border="0" cellspacing="0" cellpadding="0">
      <tr>
        <td><img src="../images/titleL3_point.gif" width="33" height="19"><span class="txt_title3">操作日志列表</span>
