...
paytype==3：财付通
 ...
 function submitform(paymethod,paytype){
    if("vmoney"==paymethod){
      ...
      var info="";
      if(paytype==1){//支付宝支付
      	url='http://<%=request.getServerName() %>:<%=request.getServerPort() %>/interface/Alipay_chajia?'
        info='helpername=<ww:property value="#request.ordermap.billname"/>&orderid=<ww:property value="#request.ordermap.orderid"/>&scangid=<ww:property value="#request.ordermap.scangid"/>';
        }
