    ...
    if("vmoney"==paymethod){
        <ww:if test="#session.loginuser!=null">
          ...
          return;
	         ...
	         type:"POST",
	         async: false,
	         url:"login!Valada_pwd.action",
	           data:{PayPwd:$("#paypwd").val()},
	         success:function(data){
	         				if(data!=null&&data=='-1'){
	         					ispay="-1";
			       	 			alert("首次使用,请先修改支付密码!当前默认为登录密码!支付密码不可与登录密码一致!");
			       	 			window.location.href="login!toeditpaypwd.action";
			       	 			return;
	         				}
	         				if(data=='-2'){
	         					ispay="-1";
      ...
      var info="";
      if(paytype==1){//支付宝支付
      	url='http://<%=request.getServerName() %>:<%=request.getServerPort() %>/interface/Liantuopay?'
        info='helpername=<ww:property value="#request.ordermap.billname"/>&orderid=<ww:property value="#request.ordermap.orderid"/>&paymethod=alipay';
        }
      if(paytype==2){//快钱支付
       url='http://<%=request.getServerName() %>:<%=request.getServerPort() %>/interface/Liantuopay?'
       ...
       if(paytype==3){//财付通支付
       url='http://<%=request.getServerName() %>:<%=request.getServerPort() %>/interface/Liantuopay?'
       info='helpername=<ww:property value="#request.ordermap.billname"/>&orderid=<ww:property value="#request.ordermap.orderid"/>&ordertype=1&paymethod=tenpay';
       }
           ...
           <!--网上银行-->
            ...
            <!--虚拟账户-->
              <div class="boxnei" id='vmoney' style="display:none">
                   <ul>
                   <li>支付密码:<input type="password" id="paypwd" name="paypwd" size="20" maxlength="20" /></li>
               ...
               <div class="nohave">&nbsp;</div>
               <div class="nohave"><input type="text" id="paypwd" name="paypwd" size="20" maxlength="20" /></div>
