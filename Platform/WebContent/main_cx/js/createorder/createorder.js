...
function bindcurrentpolicy(policyid) {
    ...
    for (s = 0; s < segarr.length; s++) {
        ...
        for (var i = 0; i < jsonList.segmentinfos.length; i++) {
            ...
            jsonList.segmentinfos[i]["saveprice"] = $("#policysaveprice_" + policyid).val();
    ...
    $("#hidbackpointinfo").val($("#policybackpoint_" + policyid).val());
    $("#span_agent_waring").html("<br/><br/>此供应商的工作时间为：<b><font color='red'>" + $("#policyworktime_" + policyid).val() + "</font></b> &nbsp;<b>改期、签转，如产生代理费差异，需回收！</b>");
}
//获取政策信息
...
function checkdata(stepindex) {
    ...
    //验证出生日期
    $("select[id*='ddlpassengertype_']").each(function (i) {
        if ($(this).val() == "2" || $(this).val() == "3") {
            var strid = $(this).attr("id");
            id = strid.replace("ddlpassengertype_", "");
            if ($("#txt_birthday_" + id).val() == "") {
                isrightdate = false;
                index = i + 1;
                j = i;
            }
    ...
    // 判断是否既有成人又有儿童票
    if (parseInt($("#hidadultpasscount").val()) > 0 && parseInt($("#hidchildpasscount").val()) > 0) {
        alert("抱歉，您的订单不能提交！\r\n预订有成人陪伴儿童订单。请分别预订成人订单和儿童订单！");
        return false;
    }
    // 首都航空只能单人单航段入库
    if (parseInt($("#hidadultpasscount").val()) > 1 && ('JD' == Aircomapnycode)) {
        alert("抱歉，您的订单不能提交！\r\nJD航空只允许单人单航段创建订单！");
        return false;
    }
    if (parseInt($("#hidchildpasscount").val()) > 0) {
        if ((cabincode == "Y" || cabincode == "F" || cabincode == "C") || (Aircomapnycode == "CZ") && cabincode == "W") {
        } else {
...
function bindpassengerinfo() {
    ...
    $("div[id*='divinformation_']").each(function (i) {
        ...
        jsonstring += '{ ID: "currentindex",Name:"' + name + '",Type:"' + type + '",Idcardtype:"' + idcardtype + '",Idcardnumber:"' + idcardnumber + '",Insurancenum:"' + insurancenum + '",Insurancemoney:"' + insurancetotalprice + '",Ticketprice:"' + ticketprice + '",Airportprice:"' + airportprice + '",Fuelprice:"' + fuelprice + '",Totalprice:"' + totalprice + '",Issave:"' + issave + '",Birthday:"' + birthday + '" },';
...
function showcommonpassenger() {
    $.ajax({
        ...
        type: "POST",
        data: { "action": "getcustomerpassenger", "Name": $("#txtsearchname").val(), "IdCardNumber": $("#txtsearchcard").val(), "Mobile": "" },
        success: function (data) {
