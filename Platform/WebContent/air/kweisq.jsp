        ...
        items: [
		     
		    {
				...
				handler : function(item){
					window.location.href="kweisq!toadd.action?<ww:property value="url"/>";
			...
			,{  
				...
				handler : function(item){
						document.form1.action="kweisq!toedit.action?id="+selectId;
            ...
            },
			{
				...
				handler : function(item){
					Ext.MessageBox.show({title:'删除',width:300, msg:'确认删除吗？',buttons: Ext.MessageBox.YESNO,fn:function(btn){
						...
						if(btn=='yes')
						{
						document.form1.action="kweisq!delete.action?id="+selectId;
						document.form1.submit();
			...
			'-',
			{
				...
				handler :function(item){
						document.form1.action="kweisq!tocheck.action?id="+selectId;
						document.form1.submit();
...
<table width="100%" border="0" cellpadding="0" cellspacing="0"
	...
	<tr>
		<td valign="top">
		<form name="form1" id="form1" method="post" action="kweisq.action">
        ...
        --></td>
										...
										<ww:iterator value="listKweisq">
											<tr id="<ww:property value="id"/>" align="center"
												...
												--><td class="table_color color_b3"><ww:property value="getagentname(angenid)" /></td>
													...
													<td class="table_color color_b3"><ww:property value="formatTimestamp(getkstime(kweiid))" /></td>
												...
												<ww:if test="status==3">审核不通过</ww:if></td>
												<!--<td class="table_color color_b3"><ww:property value="kweiid" /></td>
												
												--><td class="table_color color_b3" width="60">
												 <ww:if test="status==2">
												 <a href="kweisq!togoumai.action?kid=<ww:property value="kweiid" />&ksid=<ww:property value="id" />" style="color:#0000ff" >购买 </a>
												  <br/></ww:if>
												  <a href="kweisq!toinfo.action?kid=<ww:property value="kweiid" />&ksid=<ww:property value="id" />" style="color:#0000ff" >查看详情 </a>
												  </td>
				...
				<ww:property value="getPagination('\"kweisq!tomy.action?pageinfo.pagenum=\"+pageno')"/>
 ...
 function updateItem(){
			...
			if(length== undefined){
				...
				if(document.form1.selectid.checked ==true)
				{
						...
						document.form1.action="kweisq!toedit.action?id="+document.form1.selectid.value;
						document.form1.submit();
			  	 ...
			  	 if ( length == null || length==0 ){
					     ...
					     for (var i = 0; i < length; i++)
					      ...
					      if(len==1){
					      		document.form1.action="kweisq!toedit.action?id="+uvalue;
								document.form1.submit();
								return;
					      }else if (len>1){
 ...
 function checkItem(){
			...
			if(length== undefined){
				...
				if(document.form1.selectid.checked ==true)
				{
						...
						document.form1.action="kweisq!tocheck.action?id="+document.form1.selectid.value;
						document.form1.submit();
			  	 ...
			  	 if ( length == null || length==0 ){
					     ...
					     for (var i = 0; i < length; i++)
					      ...
					      if(len==1){
					      		document.form1.action="kweisq!tocheck.action?id="+uvalue;
								document.form1.submit();
								return;
					      }else if (len>1){
					      	var temp = confirm('只能选择一项进行操作？');
