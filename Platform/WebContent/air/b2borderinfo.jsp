...
function isOperating_new(orderid){
var ret;
   $.ajax({
    type:"POST",
    url:"passenger!ajaxValadateSuoDan.action",
...
function valadateName_new(orderid){
var ret;
   $.ajax({
    type:"POST",
    url:"passenger!ValadatePassName.action",
    data:{strTuiOrderID:orderid},
    async:false,         
    success:function(data){
     ret=data;
     ...
     if(checkfalg=="true")
     {
       ...
       if($("#txttuifee").val()=="")
       {
	    ...
	    if(confirm('您确定要对选中的乘机人进行申请改签操作吗？'))
	    {
	       ...
	       var changedate2=encodeURI(document.getElementById("txtchangedate2").value);
	       if(changedate==''){
	       alert("起飞时间不能为空!");
	       return;
	       }
	       ...
	       if(changeflight.length>6){
	       alert("航班号格式错误!");
	       return;
	       }
	       
	       var changepnr=encodeURI(document.getElementById("txtchangepnr").value);
	       window.location.href='b2bticketorder!editorderstatus.action?id='+id+'&orderstatus=13&no=2&pr=1&passid='+passengerids+'&tui_state='+ddlreasonid+'&tui_iscabinsite='+iscabinsite+'&tui_tuifeidesc='+tuifeidesc+'&tui='+$("#txttuifee").val()+'&changedate='+changedate+'&changeflight='+changeflight+'&changecabin='+changecabin+'&changepnr='+changepnr+'&changedate2='+changedate2;
	    }
     }
...
function goEdit2(ur,id){
  var states=ur.split("orderstatus=");
    ...
    var retvalue=isOperating_new(id);
  ...
  if(true){
     var retvalue= isOperating_new(id);
       ...
       //验证时间
      		 ...
      		 var createtime=$("#createtime"+id).html();
      		 var checktime='30';
            var tes=valadatePayTime(id,checktime);
            if(tes=='ok'){
            window.location.href="b2bticketorder!topayorder.action?id="+id;
            }else{
            	if(!confirm("创建订单时间超过"+checktime+"分钟,已不保证位置存在!!!请致电客服咨询,点击确定将继续支付!!")) {
...
function valadatePayTime(orderid,ctime){
var ret="-1";
   $.ajax({
    type:"POST",
    url:"passenger!checkPayTimeValite.action",
    data:{strTuiOrderID:orderid,checktime:ctime},
    async:false,         
    success:function(data){
     ret=data;
    }            
...
<body>
<form name="form1" method="post" action="b2bticketorder.action">
<div id="divpassenger"
													...
													<tbody><tr>
														...
														<td align="left">
															<ul class="ul" style="width: 99%; position: relative; left: 1px;">
																...
																<li id="li1"  <ww:if test="s_orderstatus==1">class="xuanzhong"</ww:if>><a style="cursor: pointer"   href="b2bticketorder.action?s_orderstatus=1" target="member">等待支付</a></li>
																<li id="li2"  <ww:if test="s_orderstatus==2">class="xuanzhong"</ww:if>><a style="cursor: pointer"  href="b2bticketorder.action?s_orderstatus=2" target="member">等待出票</a></li>
																<li id="li3" <ww:if test="s_orderstatus==3">class="xuanzhong"</ww:if> ><a style="cursor: pointer"  href="b2bticketorder.action?s_orderstatus=3" target="member">出票完成</a></li>
																<li id="li19" <ww:if test="s_orderstatus==19">class="xuanzhong"</ww:if>  ><a style="cursor: pointer"  href="b2bticketorder.action?s_orderstatus=19" target="member"">拒单-暂未退款</a></li>
																<li id="li19" <ww:if test="s_orderstatus==20">class="xuanzhong"</ww:if>  ><a style="cursor: pointer"  href="b2bticketorder.action?s_orderstatus=20" target="member"">拒单-已经退款</a></li>
																<li id="li-1" <ww:if test="s_orderstatus==6">class="xuanzhong"</ww:if> ><a style="cursor: pointer" href="b2bticketorder.action?s_orderstatus=6">已取消订单</a></li>
																<li id="li-1" <ww:if test="s_orderstatus==0&&lockorder=='false'">class="xuanzhong"</ww:if> ><a style="cursor: pointer" href="b2bticketorder.action">所有订单</a></li>
																<ww:if test="getLoginUserAgent().agenttype==1">
																<li id="li-1" <ww:if test="lockorder=='true'">class="xuanzhong"</ww:if> ><a style="cursor: pointer" href="b2bticketorder.action?lockorder=true">订单解锁</a></li>
																</ww:if>
															</ul>
						...
						<table width="100%" border="0" align="center" cellpadding="0"
								...
								<table id="menutable" width="99%" border="1" align="center"
									...
									<tbody>
										<tr class="tbody_color">
											...
											<th class="table_color"><b>起飞/预订/支付时间</b></th>	
											...
											<ww:if test="getLoginUserAgent().agenttype==1||getLoginUserAgent().agenttype==3">
										...
										<ww:iterator value="listOrderinfo">
											...
											<tr id="<ww:property value="id"/>" align="center"
												...
												<div style="position: relative;">
													...
													<br/>出:
														<ww:if test="userid!=null&&userid!=0&&userid!=-1">
														
														<font color='red'><b><ww:property value='getemployeenamebyid(userid)' /></b></font>
														</ww:if><ww:else>
														暂无处理人
														</ww:else>
												...
												<span id='departtime<ww:property value="id"/>'><ww:property
													...
													<br />
												<span id='createtime<ww:property value="id"/>'><ww:property value="formatTimestamptoMinute(createtime)" /></span>
												<br />
												<span style="color: red">
												<ww:property value="#test.GetPayTimeByOrdernumber(ordernumber)"/>
												</span>
												</td>
												...
												<!-- 结算价 -->
												￥<ww:property
														value="formatMoneyShort(converNull(currplatfee,0)+converNull(postmoney,0)+converNull(totalticketprice,0)+converNull(totalairportfee,0)+converNull(totalfuelfee,0)+converNull(getIssurByOrderid(id),0)+converNull(getxuanzuoByOrderid(id),0))" />
														</br>
														...
														<ww:if test="getxuanzuoByOrderid(id)>0"></br><span style="color: red">(含选座)</span></ww:if>
													</td>
												...
												<td class="table_color color_b3"  >
												 ...
												 <br/>
												  <ww:if test="orderstatus==19"><a href="#" style="color: red" onclick="loddesc(<ww:property value="id" />);">查看原因</a> <br/></ww:if>
												  <input type="hidden" id="orderid_<ww:property value="id" />" value="<ww:property value="addresa" />" />
												  
												 <ww:if test="orderstatus==4||orderstatus==5">
														<ww:property value="#test.gettuifeiinfo(id)"/>
												  </ww:if>
													...
													<!-- 支付状态 -->
												<td class="table_color color_b3" ><ww:property value="getPayStatus(paystatus)" /></td>
												
												 <ww:if test="getLoginsessionagent().agenttype==1">
												   <td class="table_color color_b3" >
												   <!-- <ww:property value="getExtStateToString(extorderstatus)" /> -->
													<font style="color: red">供应状态:
													<ww:property value="getExtStateToString(extorderstatus)" />
													</font>
													</td>
													</ww:if>
												...
												buypolicy==1 自己的政策
												-->
												<input type="button" class="button108" id="btnCancel" value="发送短信"  onclick="ToOnyUrl('ymsend!toadd2.action?orderinfoid=<ww:property value='id' />');" /> <br />
												
												<input type="button" class="button108" id="btnCancel" value="支付通知"  onclick='tongzhi_pay(<ww:property value="id" />);' /> <br />
												<input type="button" class="button108" id="btnCancel" value="出票通知"  onclick='tongzhi_chu(<ww:property value="id" />);' /> <br />
												
												<input type="button" class="button108" id="btnCancel" value="提交证明"  onclick="ToOnyUrl('airdelayprove!toadd.action?s_orderid=<ww:property value='id' />');" /> <br />
												<input type="button" class="button108" id="btnCancel" value="查看证明"  onclick="ToOnyUrl('airdelayprove.action?s_orderid=<ww:property value='id' />');" /> <br />
												...
												<input type="button" class="button108" id='nextorder<ww:property value="id"/>' value="创建PNR" onclick='CreatePnrByID(<ww:property value="id" />);' /><br/>	
												 
												<input type="button" class="button108" id='cnbtnpay<ww:property value="id"/>' value="锁单" onclick='suodan(<ww:property value="id" />);' /><br/>
											 	
											 	
												<input type="button" class="button108" id='cnbtnpay<ww:property value="id"/>' value="支付供应" onclick='topay(<ww:property value="id" />,<ww:property value="policyagentid" />);' /><br/>
												...
												<input type="button" class="button108" id='baoxian' value="选择下单" onclick="ChangAgentByOrder(<ww:property value="id" />);" /><br/>
												...
												<!-- 测试用 -->
												<input type="button" class="button108" id="btnCancel" value="生成差价单"  onclick="AddChaJia(<ww:property value='id' />);" /> <br />
												<input type="button" class="button108" id="btnCancel" value="在线补差价"  onclick="ToOnyUrl('scang.action?orderinfoid=<ww:property value='id' />');" /> <br />
												...
												<ww:if test="ratevalue>0&&paystatus==1">
												<input type="button" class="button108" id='baoxian' value="添加儿童票" onclick="ToOnyUrl('b2bticketorder!toaddRTticket.action?s_orderid=<ww:property value="id" />');" /><br/>
												<input type="button" class="button108" id='baoxian' value="添加婴儿票" onclick="ToOnyUrl('b2bticketorder!toaddYEticket.action?s_orderid=<ww:property value="id" />');" /><br/>
												...
												<ww:if test="orderstatus==3&&receipt==1">
												<input type="button" class="button108" id="btnRRTicket1" value="打印信息单"   onclick="popup('passenger!orderchupiao2.action?orderinfoid=<ww:property value='id' />');" /> <br />
												<input type="button" class="button108" id="btnRRTicket1" value="打印行程单"   onclick="popup('passenger!orderchupiao.action?orderinfoid=<ww:property value='id' />');" /> <br />
												</ww:if>
											   ...
											   <ww:if test="(orderstatus==19||orderstatus==27||orderstatus<3||(orderstatus==1&&paymethod!=3))&&(#buyln==1||#agenttype==1)">
											    ...
											    <ww:if test="orderstatus==27&&#agenttype==1||orderstatus==2">
											      <!-- 订单确认 国际和国内分别处理 -->
											      <ww:if test="internal==1">
												    <input type="button" class="button108" id="btnCancel" value="订单确认"  onclick="interorderconfirm(<ww:property value="id" />);" /><br/>
												  </ww:if>
												  <ww:else>
												    <input type="button" class="button108" id="btnCancel" value="订单确认"  onclick="goEdit('b2bticketorder!editorderstatus.action?id=<ww:property value="id" />&orderstatus=1','<ww:property value="id" />')" /><br/>
												  </ww:else>
											    ...
											    <ww:if test="extorderid!=null&&paystatus==1&&#agenttype==1&&extorderstatus==1&&orderstatus<3">
											     <input type="button" class="button108" id='cnbtnpay<ww:property value="id"/>' value="支付供应" onclick='topay(<ww:property value="id" />,<ww:property value="policyagentid" />);' /><br/>
											 	 </ww:if>
											  ...
											  <ww:if test="#agenttype==1&&extorderid==null&&orderstatus<3">
											     <input type="button" class="button108" id='nextorder<ww:property value="id"/>' value="重新下单" onclick='nextOrderticket(<ww:property value="id" />);' /><br/>						
											  </ww:if>
                                            ...
                                            //26升舱失败";27待确认";28在途订单";29待收银";30申请换开";											
												...
												<ww:if test="#test.checkFeiPiaoValite(createtime)">
												...
												<input type="button" class="button108" id="btnCancel" value="发送短信"  onclick="ToOnyUrl('ymsend!toadd.action?orderinfoid=<ww:property value='id' />');" /> <br />
							...
							<tr>
								<td height="43" align="center">
								  <ww:if test="s_send==1">
								    <ww:property value="getPagination('\"b2bticketorder.action?s_orderstatus=3&s_send=1&pageinfo.pagenum=\"+pageno')"/>
								  </ww:if>
								  <ww:elseif test="s_money==1 && s_send==1">
								    <ww:property value="getPagination('\"b2bticketorder.action?s_send=1&s_money=1&pageinfo.pagenum=\"+pageno')"/>
								  </ww:elseif>
								  <ww:else>
								    <ww:property value="getPagination('\"b2bticketorder.action?pageinfo.pagenum=\"+pageno')"/>
								  </ww:else>
...
alert(retvalue);
 ...
 function shows(id,agentid){
   ...
   $("#divpassenger").dialog("open");
		window.location.href="b2bticketorder!changagent.action?id="+id;
	}else{
		alert(retvalue);
		}
   ...
   $("#divpassenger").dialog("open");
		window.location.href="scang!toadd.action?orderinfoid="+id;
	}else{
		alert(retvalue);
		}
...
function deleteItem(){
	 ...
	 function alipaytui(orderid){
		...
		$.ajax({
	         type:"POST",
	         async:false,
	         url:"passenger!alipay_tuifenrun.action?rndmath="+Math.floor(Math.random()*100),
	         data:{strTuiOrderID:orderid},
	         success:function(data){
	         	if(data){
		         	if(data=='-1'){
	 ...
	 function suodan(orderid){
		...
		$.ajax({
	         type:"POST",
	         async:false,
	         url:"passenger!suodan.action?rndmath="+Math.floor(Math.random()*100),
	         data:{strTuiOrderID:orderid},
	         success:function(data){
	         	alert(data);
  ...
  var retname="-1";
     retname=valadateName_new(orderid);
	...
	if(retname=='-1'){
	 ...
	 function tongzhi_pay(orderid){
		 
		$.ajax({
	         type:"POST",
	         async:false,
	         url:"passenger!tongzhi_pay.action?rndmath="+Math.floor(Math.random()*100),
	         data:{strTuiOrderID:orderid},
	         success:function(data){
	         	alert(data);
	...
	function tongzhi_chu(orderid){
		 
		$.ajax({
	         type:"POST",
	         async:false,
	         url:"passenger!tongzhi_chu.action?rndmath="+Math.floor(Math.random()*100),
	         data:{strTuiOrderID:orderid},
	         success:function(data){
	         	alert(data);
	...
	function CreatePnrByID(orderid){
		 
		$.ajax({
	         type:"POST",
	         async:false,
	         url:"passenger!CreatePnrByID.action?rndmath="+Math.floor(Math.random()*100),
	         data:{strTuiOrderID:orderid},
	         success:function(data){
	         	//alert(data);
