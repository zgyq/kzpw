...
//**Edited By:sunbin***//
//**Date:2011-06-13***//
/**dataType:调取数据的类型，0=酒店和租车的城市数据,1=酒店城市数据(旅游城市数据同酒店城市数据一样),2=租车城市数据***/

var varAddress="";
var varAddresscar="";
function loadCityData_Common(dataType)
{
   var urlactionhotel="hoteluserbook!getCityAirPortData.action";
   var urlactioncar="hotelsubscribe!getCityAirPortDataToCar.jspx";
   var url="";
...
//生成并显示出城市下拉菜单
function GetCityList(fctThisObj,dataType,txtname,hidID){
   ...
   //隐藏区域
	...
	$("#index_regiondiv_e").hide();
	
	
//	


//alert("txtname=="+txtname+"--hidID=="+hidID);
	...
	if(dataType=="1")
	{
	   varNewCityData=varAddress;
	}
	else if(dataType=="2")
	{
	  varNewCityData=varAddresscar;
	}
	//清除已选城市Value
	...
	var varData=(varObjValue==""?(varThisObjAdd==""?varNewCityData:varThisObjAdd):varNewCityData);
	//varAddress="北京#beijing%bj&607,";
	...
	for(var i=0;i<varCityDataSplit.length-1;i++){
		...
		if(varCityDataSplitI.toUpperCase().indexOf(varObjValue.toUpperCase())>=0 || varObjValue=="" || i==varObjValue){ // || varCityDataSplitI.toLowerCase().indexOf(varObjValue.toLowerCase())>=0
			...
			if(varThisPageI>varPageId*varPageRCount && varThisPageI<=(varPageId+1)*varPageRCount){
				...
				varHtmlStr+="<a href='javascript:;' onclick='WriteCity(\""+varCityDataSplitI.split("&")[1]+"|"+varCityDataSplitISplit[0]+"|"+txtname+"|"+hidID+"\")' id='menuA"+varThisPageI+"' title='"+varCityDataSplitI.split("%")[0].split("#")[1] +'|'+varCityDataSplitISplit[0] +"'><span id='menuA"+varThisPageI+"'9'>"+varCityDataSplitISplit[0]+"&nbsp;&nbsp;</span>"+varCityDataSplitI.split("%")[0].split("#")[1]+"</a>";
			}
		}
	}
...
function WriteCity(fctI){
	...
	$("#"+varMenuValue[3]).val(varMenuValue[0]);
	$("#"+varMenuValue[2]).val(varMenuValue[1]);
}
//隐藏城市列表
function _Hidden(e){
	e=e?e:event;
