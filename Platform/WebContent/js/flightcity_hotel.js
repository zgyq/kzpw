...
function LoadCityData(w)
{
    ...
    if(w==1){
     ...
     var HttpUrl="hotelsubscribe!getCityAirPortData.action?rndmath="+Math.floor(Math.random()*100);
    }
    if(w==2){
   // alert("w=2");
     document.getElementById("guojia").value=2;
    var HttpUrl="hotelsubscribe!geta.action?rndmath="+Math.floor(Math.random()*100);
    }
