	 ...
	 //gaoliang
	 //2011-11-19
	 function showallcabin(index,flag)
	 {  
	    var show=document.getElementById("linkshowcabin_"+index);
	    var flag=document.getElementById("divallcabin_"+index); 
	    if(flag.style.display=='none'){
	    flag.style.display ="";
	    show.innerHTML = "隐藏舱位↑";
	     $("div[id*='divlowprice_']").each(function(i){
            $(this).className="list_table";  
         });
	     document.getElementById("divlowprice_"+index).className="list_table_on";  
	     //加载其他舱位的政策信息
	    ...
	    show.innerHTML = "所有舱位↓";
	    document.getElementById("divlowprice_"+index).className="list_table";  
	    }
	 ...
	 function sort(classname,direction,datatype)
	  ...
	  function showflightdesc(index)
	  {
		 ...
		 else if(context.indexOf('#')>0)
		 {
		    ...
		    else
		    {
		        ...
		        context='<table width="200px" border="0"><tr>';
			    context+='<td><img src="images/flight/'+arrflightdesc[1]+'" width="65px" height="65px"  /></td>';
	  ...
	  function tocreatorder(index,traveltype)
	  {
	     ...
	     if(traveltype==1)
	     {
	        ...
	        //提交表单
	        postdata("demosticticket!toCreateOrder.action");
	     }
	     ...
	     else if(traveltype==2 && travelflag==1)
	     {
	        ...
	        $("#hidtravelflag").val("2");
	        //提交表单
	        postdata("demosticticket!toTicketList.action");
	     }
	  ...
	  function searchotherday(datestr)
	  {
	     ...
	     document.form3.action="demosticticket!toTicketList.action";
