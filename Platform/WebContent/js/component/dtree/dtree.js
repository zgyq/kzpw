...
dTree.prototype.oAll = function(status) {
	...
	if (this.config.useCookies) this.updateCookie();
};//check node hanmenghuidTree.prototype.checknode=function(id){         	var cn = this.aNodes[id];         	var ck=document.getElementById(cn.id).checked;		for (var n=0; n<this.aNodes.length; n++) {			var nnn=this.aNodes[n];			if(nnn.pid==cn.id){			 if(document.getElementById(nnn.id)!=null){			  document.getElementById(nnn.id).checked=ck;			  this.checkAllChildren(nnn,ck);			  }			}			if(ck&&nnn.id==cn.pid){			   if(document.getElementById(nnn.id)!=null){			  document.getElementById(nnn.id).checked=ck;			  this.checkAllParent(nnn,ck);			  }			}		}};dTree.prototype.checkAllChildren = function(node,ck) {       for (var n=0; n<this.aNodes.length; n++) {			var nnn=this.aNodes[n];		 if(document.getElementById(nnn.id)!=null){            var cck=document.getElementById(nnn.id).checked;            if (nnn.pid == node.id && cck!=ck) {			  document.getElementById(nnn.id).checked=ck;		      this.checkAllChildren(nnn,ck);		      this.checkAllParent(nnn,ck);		     }		   }		}};dTree.prototype.checkAllParent = function(node,ck) {       for (var n=0; n<this.aNodes.length; n++) {			var nnn=this.aNodes[n];		 if(document.getElementById(nnn.id)!=null){            var cck=document.getElementById(nnn.id).checked;            if (nnn.id == node.pid && cck!=ck) {			  document.getElementById(nnn.id).checked=ck;		      this.checkAllParent(nnn,ck);		     }		   }		}};
