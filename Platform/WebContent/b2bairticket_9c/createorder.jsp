...
<ww:iterator value="listsegmentinfo" status="index">
		...
		<tr>
			...
			<td rowspan="2"><ww:property value="ratevalue" />/<ww:property value="Mathfloor(ratevalue*parvalue/100)" /></td>
			<td rowspan="2"><ww:property value="formatMoneyToInt(airportfee+fuelfee+parvalue-Mathfloor(ratevalue*parvalue/100))" /></td>
...
<div class="tianxie"><font class="f font-fff-14b">填写乘客信息</font><b><font style="color: red;" class="f font-fff-14b">春秋航空政策备注：此政策需申请换舱位出票，出

票后有差价，任何情况下差价不退，不能打印行程单，一经出票百分百保证行程</font></b><font
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
	<tbody>
		...
		<tr>
			...
			<td width="20%">证件类型</td>
			<td width="20%">证件号</td>
			<td width="10%" class="document_back">保险</td>
			<td width="10%" class="document_back">选座</td>
			<!--
			...
			<ww:if test="listsegmentinfo.get(0).discount<60">
			...
			</select><ww:property value="baoxianprice" />元/份</td>
			</ww:if><ww:else>
			<td class="document_back"><select onchange="changbaoxian();"
				name="passengerLists[0].insurance" id="ddlinsurance_<ww:property value="#state.index"/>">
			...
			<td class="document_back">
			
			
			<ww:if test="listsegmentinfo.get(0).discount<600">
			<select onchange="subprice();"
				name="passengerLists[0].xuanzuo" id="ddlxuanzuo_<ww:property value="#state.index"/>">
				<option value="1">1份</option>
			</select>10元/份
			</ww:if><ww:else>
			<select onchange="subprice();"
...
var baoxianprice=0;//保险价格
	...
	baoxianprice+=<ww:property value="baoxianprice" />;
	
	
	$("#hid_onezrateid").val("1");
    $("#hid_onezratevalue").val(<ww:property value="ratevalue" />);
...
function subprice(){
  ...
  var zrateoneid=GetRadioValue("zrate_one");
   ...
   //alert("zrateoneid:"+zrateoneid);
     ...
     onezrateid="1";
     onezratevalue='<ww:property value="listsegmentinfo.get(0).ratevalue" />';
     oneprice='<ww:property value="listsegmentinfo.get(0).parvalue" />';
    
     onejiesuanjia='<ww:property value="listsegmentinfo.get(0).price" />';
     oneyouhui=parseInt(oneprice)-parseInt(onejiesuanjia);
 ...
 var  allxuanzuorenshu=0;//选座总份数
 	...
 	//alert(onejiesuanjia+"-"+jijian+"-"+ranyou+"-"+renshu+"-"+allbaoxianrenshu+"-"+baoxianprice);
 	...
 	//alert(ptfei);
 	var all_totalprice=((parseInt(onejiesuanjia)+parseInt(jijian)+parseInt(ranyou))*renshu)+parseInt(allbaoxianrenshu)*parseInt(baoxianprice)+parseInt(allxuanzuorenshu)*parseInt(xuanzuoprice)+parseInt(ptfei);
   ...
   //  alert("onezrateid:"+onezrateid+",onezratevalue:"+onezratevalue+",onejiesuanjia:"+onejiesuanjia+",oneyouhui:"+oneyouhui);
	   ...
	   var issave=1;
           ...
           // shengri=$("#s_txtBirthday_"+id).val();
             ...
             var xzprice=parseInt($("#ddlxuanzuo_"+id).val())*parseInt(xuanzuoprice);
            
            jsonstring+='{ ID: "currentindex",Name:"'+name+'",Type:"'+type+'",Idcardtype:"'+idcardtype+'",Idcardnumber:"'+idcardnumber+'",Insurancenum:"'+insurancenum+'",Insurancemoney:"'+baoxianprice*insurancenum+'",xuanzuoprice:"'+xzprice+'",Ticketprice:"'+ticketprice+'",Airportprice:"'+airportprice+'",Fuelprice:"'+fuelprice+'",Totalprice:"'+totalprice+'",Issave:"'+issave+'",shengri:"'+shengri+'" },';
     ...
     for(i=0;i<=8;i++){
       ...
       if(1==1){
			...
			document.form2.action="b2b9cairticket!createorder.action";
