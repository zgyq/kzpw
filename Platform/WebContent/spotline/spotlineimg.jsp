        ...
        items: [
		     
		    {
				...
				handler : function(item){
					window.location.href="spotlineimg!toadd.action?<ww:property value="url"/>&s_spotlineid=<ww:property value="s_spotlineid"/>";
			...
			,{  
				...
				handler : function(item){
						document.form1.action="spotlineimg!toedit.action?id="+selectId;
            ...
            },
			{
				...
				handler : function(item){
					Ext.MessageBox.show({title:'删除',width:300, msg:'确认删除吗？',buttons: Ext.MessageBox.YESNO,fn:function(btn){
						...
						if(btn=='yes')
						{
						document.form1.action="spotlineimg!delete.action?id="+selectId;
						document.form1.submit();
			...
			'-',
			{
				...
				handler :function(item){
						document.form1.action="spotlineimg!tocheck.action?id="+selectId;
						document.form1.submit();
...
<table width="100%" border="0" cellpadding="0" cellspacing="0"
	...
	<tr>
		<td width="100%" height="29" class="box-bottom bg"><b
			class="anse"><span class="font-blue-cu">&nbsp;&nbsp;&nbsp;景区线路图片信息列表</span></b></td>
	</tr>
	<tr>
		<td valign="top">
		<form name="form1" method="post" action="spotlineimg.action">
	<input type="hidden" name="s_spotlineid" value='<ww:property value="s_spotlineid"/>' />
		<table width="100%" border="0" cellpadding="0" cellspacing="0"
			align="center">
			<tr>
				<td valign="top">
							...
							<tr>
								...
								<table id="menutable" width="99%" border="1" align="center"
									...
									<tbody>
										...
										<ww:iterator value="listSpotlineimg">
...
<script language="JavaScript">
	function toadd(){
		window.location="spotlineimg!toadd.action?<ww:property value="url"/>&s_spotlineid=<ww:property value="s_spotlineid"/>";
	}
	function deleteItem(){
			...
			if(length== undefined){
				...
				if(document.form1.selectid.checked ==true)
				{
						...
						if(temp==true){
							...
							document.form1.action="spotlineimg!delete.action?id="+document.form1.selectid.value;
							document.form1.submit();
						}
						return;
				}
			  	 ...
			  	 if ( length == null || length==0 ){
					     ...
					     for (var i = 0; i < length; i++)
					      ...
					      if(len==1){
					      	var temp = confirm('确认删除吗？');
							if(temp==true){
								document.form1.action="spotlineimg!delete.action?id="+uvalue;
								document.form1.submit();
							}
							return;
					      }else if (len>1){
					      	var temp = confirm('确认删除吗？');
					      	if(temp==true){
					      		document.form1.action="spotlineimg!batch.action?opt=1";
								document.form1.submit();
							}
							return;
 ...
 function updateItem(){
			...
			if(length== undefined){
				...
				if(document.form1.selectid.checked ==true)
				{
						...
						document.form1.action="spotlineimg!toedit.action?id="+document.form1.selectid.value;
						document.form1.submit();
			  	 ...
			  	 if ( length == null || length==0 ){
					     ...
					     for (var i = 0; i < length; i++)
					      ...
					      if(len==1){
					      		document.form1.action="spotlineimg!toedit.action?id="+uvalue;
								document.form1.submit();
								return;
					      }else if (len>1){
 ...
 function checkItem(){
			...
			if(length== undefined){
				...
				if(document.form1.selectid.checked ==true)
				{
						...
						document.form1.action="spotlineimg!tocheck.action?id="+document.form1.selectid.value;
						document.form1.submit();
			  	 ...
			  	 if ( length == null || length==0 ){
					     ...
					     for (var i = 0; i < length; i++)
					      ...
					      if(len==1){
					      		document.form1.action="spotlineimg!tocheck.action?id="+uvalue;
								document.form1.submit();
								return;
					      }else if (len>1){
					      	var temp = confirm('只能选择一项进行操作？');
