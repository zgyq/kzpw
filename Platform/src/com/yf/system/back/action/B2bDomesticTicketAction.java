...
public class B2bDomesticTicketAction extends B2b2cbackAction {
	...
	private int s_seatype=0;//0是普通  1是多网
	
	// 航空公司信息
	public List<Aircompany> listAirCompany;
	// 航班信息List
	private List<FlightInfo> listFlightInfoAll=new ArrayList<FlightInfo>();
	// 航班信息List
	...
	public  B2bDomesticTicketAction() {
		...
		public List<Zrate> Getallzrate_(String z_startcity, String z_endcity,
			...
			if (intflag == 2) {
			 ...
			 for(int a=0;a<listzrateJR.size();a++){
			  ...
			  //ZrateServer.getInstance().setUrl("http://222.73.105.197:28080");
			...
			//System.out.println(ZrateServer.getInstance().getUrl());
			  System.out.println("本地政策:"+listzrate.size());
	...
	public String toTicketList() throws Exception {
		...
		//是否开启多网比价
		searchParam.setFromTime(s_seatype+"");
		...
		// 调用查询航班方法
		 ...
		 String isopen="close";
		 List<Sysconfig> listsys=Server.getInstance().getSystemService().findAllSysconfig(" WHERE 1=1 AND "+Sysconfig.COL_name+" ='openchunqiu'", " ORDER BY ID ", -1, 0);
			if(listsys!=null&&listsys.size()>0){
				isopen=listsys.get(0).getValue();
			}
			
			if(isopen.equals("open")){
				 try
				 {
					 listFlightInfoAll_9c = AVOpen_9c(searchParam, s_traveltype);
		         }
				 catch (Exception e) 
				 {
	...
	public String toCreateOrder_no()
	{
		
		
		System.out.println("toCreateOrder_抓取的价格--输出Json选中航程信息："+s_jasonsegmentinfo);
		//拦截登录
		Customeruser loginuser = getLoginUser();
		//ActionContext.getContext().getSession().put("pageParam",s_jasonsegmentinfo);
		//ActionContext.getContext().getSession().put("pageUrl","ticticket!toCreateOrder.jspx");
	...
	public int s_iscreatepnr=0;
	public String s_createcode="";
	...
	public String createorder()
	{
		...
		int intCreatePNRType = 0;
		
		
		
		//是否生成PNR
		int intIsCreatePNR=0;//0不生成PNR   1生成PNR
		intIsCreatePNR=s_iscreatepnr;
		...
		for(int s=0;s<listsegment.size();s++)
		{
			...
			if(intIsCreatePNR==1){//如果生成PNR
				...
				if(strpnr.equals("-1")||strpnr.equals("NO PNR")||strpnr.equals("NOPRICE")||strpnr.equals("NAMEERR")||strpnr.equals("FORMAT")||strpnr.equals("NOPNR")||strpnr.trim().length()==0){
					...
					List listonrinfo=GetRtPatPNR(strpnr, listpass);//0pnrinfo 1patinfo 2bigpnr
						...
						if(orderinfo.getPattxt().indexOf("没有符合条件的运价")!=-1){
			...
			System.out.println("onezrateid:"+onezrateid);
			System.out.println("url:"+ZrateServer.getInstance().getUrl());
			...
			if(s_createcode.equals("nozrate")){
				zrate.setRatevalue(0f);
				zrate.setAgentid(46l);
				zrate.setId(0l);
			}
			...
			orderinfo.setPostmobile(zrate.getWorktime()+"-"+zrate.getAfterworktime());//供应商工作时间
...
*/					
	...
	public String createorder_no()
	{
		...
		for(int s=0;s<listsegment.size();s++)
		{
			...
			orderinfo.setMemo(listsegment.get(s).getFlightdesc());
...
*/					
	...
	public List<Segmentinfo> getSelectedSegment(String strJson)
	{
        ...
        for(int j=0;j<ArrSegment.length;j++)
        {
			...
			for(int i=0;i<jsons.size();i++)
			{
				...
				//当前返点
				String ZrateValue="0";
				if(segmentJson.getString("parvalue")!=null&&segmentJson.getString("parvalue")!=""&&segmentJson.getString("parvalue").trim().length()>0){
				ZrateValue=segmentJson.getString("parvalue");
				}
				...
				String laiyuan="";
				if(segmentJson.toString().indexOf("isspecial")!=-1&&segmentJson.getString("isspecial")!=null){
					
					
					laiyuan=segmentJson.getString("isspecial");
					
					System.out.println("数据来源:"+laiyuan);
					String flightdesc="阿里数据";
					if(laiyuan.equals("ali")){
						flightdesc="阿里数据";
					}
					if(laiyuan.equals("qunar")){
						flightdesc="去哪数据";
					}
					segment.setFlightdesc(flightdesc);
				}
	...
	public void setListhelpcenterinfo(List<Helpcenterinfo> listhelpcenterinfo) {
		...
		public int getS_iscreatepnr() {
			return s_iscreatepnr;
		}
		public void setS_iscreatepnr(int s_iscreatepnr) {
			this.s_iscreatepnr = s_iscreatepnr;
		}
		public String getS_createcode() {
			return s_createcode;
		}
		public void setS_createcode(String s_createcode) {
			this.s_createcode = s_createcode;
		}
		public int getS_seatype() {
			return s_seatype;
		}
		public void setS_seatype(int s_seatype) {
			this.s_seatype = s_seatype;
		}
