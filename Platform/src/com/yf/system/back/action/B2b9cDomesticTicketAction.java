...
public class B2b9cDomesticTicketAction extends B2b2cbackAction {
	...
	private int s_seachtype=0;//0是普通  1是柜台
	...
	//保险费用
	private int baoxianprice = 7;
  
	public  B2b9cDomesticTicketAction() {
	...
	public int formatTimestampHH(Timestamp date){
		try {
			String ret=(new SimpleDateFormat("HH").format(date));
	...
	public Double Mathfloor(Double test){
	 ...
	 */
		...
		private String z_fandian;//传递来的当前返点
		...
		public String toRtPnr(){
			
			
			
			return "toRtPnr";
		}
		public String toGroupTicket(){
			
			
			
			return "toGroupTicket";
		}
		...
		public void GetNewvalue(){
			...
			Float fratevalue=Float.parseFloat(z_fandian);
			
			fratevalue=Getliudianvalue(Float.parseFloat(z_fandian));
			System.out.println("原始:"+z_fandian+",匹配:"+fratevalue);
			 Writer writer;
				try {
					writer = response.getWriter();
					writer.write(fratevalue+"");
					writer.flush();
					writer.close();
				} catch (IOException e) {
		...
		public void findcabinlowBY() {//listOrderPrice
		    ...
		    String isgaofan="1";
			    ...
			    System.out.println("s_cabincode:"+s_cabincode);
		...
		public void getFlightStopinfo() throws Exception {
			...
			strReturn=strStopcity+"@"+strSTime+"-"+strETime;
		...
		public void GetZrateList() throws Exception
		{
			...
			//取得政策数据
			listzrate = Getallzrate_(z_startcity, z_endcity, z_fromdate,intflag, z_aircode, z_airline, z_cabincode,stime);
			StringBuilder sb=new StringBuilder();
			...
			if(listzrate.size()>0)
			{
						...
						sb.append("<tr style='text-align: left;' bgcolor='#f9f8f8' height='40px;'>");
						...
						if(intflag==1)
						{
							...
							inputid="id='rdo_zrate1_"+a+"'";
						}
						else if(intflag==2)
						{
							tid="id='twozrate_"+z_airline+"_"+policyindex+"'";
							inputid="id='rdo_zrate2_"+a+"'";
						}
						...
						Float jiesuanjia=(Float.parseFloat(z_price)-Float.parseFloat(youhui));
						sb.append("<td>￥"+jiesuanjia+"</td>");
						...
						sb.append("<td><input  type='radio' "+inputid+"  "+strCheck+" "+zratename+" value='"+listzrate.get(a).getId()+"-"+zvalue+"-"+jiesuanjia+"-"+youhui+"' onclick=\"showRmak_('Remark_"+a+"');\"  /></td>");
						sb.append("</tr>");
						sb.append("<tr "+rmakstyle+" id='Remark_"+a+"'  >");
						String rmk="暂无特殊规定!!!";
						if(listzrate.get(a).getRemark()!=null){
						rmk=listzrate.get(a).getRemark();
						}
						sb.append("<td colspan='7' style='text-align: left;'><font color='red' size='2px'>政策备注:"+rmk+"</font></td>");
						sb.append("</tr>");
						sb.append("</tbody>");
		 ...
		 * @return 政策列表
		 */
		public List<Zrate> Getallzrate_(String z_startcity, String z_endcity,
		...
		public String GetCabinType(Float discont,boolean isprice,String cabincode){
			System.out.println("discont:"+discont+",cabin:"+cabincode);
			if(cabincode.equals("P")&&discont>200){
				return "豪华头等舱";
			}
			
			if(cabincode.equals("J")&&discont>200){
				return "豪华头等舱";
			}
			if(cabincode.equals("W")&&discont==100){
				return "高端经济舱";
			}
			if(cabincode.equals("C")&&discont>150){
				return "超值头等舱";
			}
			if(cabincode.equals("D")&&discont>100&&discont<150){
				return "公务舱";
			}
			if(cabincode.equals("D")&&discont>150){
				return "头等舱";
			}
			if(cabincode.equals("D")&&discont<100){
				return "经济舱";
			}
			if(cabincode.equals("P")&&discont>0&&discont<100){
				return "经济舱";
			}
			if(cabincode.equals("A")&&discont>150){
				return "超值头等舱";
			}
			if(cabincode.equals("A")&&discont>=100&&discont<150){
				return "公务舱";
			}
			...
			if(cabincode.equals("P")){
				return "<span style='color: red;'>超值头等舱</span>";
			}
			if(cabincode.equals("Z")&&discont>0&&discont<100){
				return "经济舱";
			}
			if(cabincode.equals("Z")&&discont>100&&discont<150){
				return "超值头等舱";
			}
			...
			if((discont<40)){
				return "<span style='color: red;'>特价经济舱</span>";
			}	
    ...
    public String getFlightnub() {
		...
		public String PnrPiPeiZrate(){
			
			
			
			
			
			return "PnrPiPeiZrate";
		}
	...
	public String toTicketList() throws Exception {
		System.out.println("s_jasonsegmentinfo:"+s_jasonsegmentinfo);
		...
		// 调用查询航班方法
		 ...
		 try
		 {
			 listFlightInfoAll = AVOpen_9c(searchParam, s_traveltype);
         }
		 catch (Exception e) 
		 {
	...
	public List<FlightInfo> AVOpen_9c(FlightSearch flightSearch, int intType) {
		...
		String zratevalue="1";
		 List<Sysconfig> listsys1=Server.getInstance().getSystemService().findAllSysconfig(" WHERE 1=1 AND "+Sysconfig.COL_name+" ='WEBZRATEVALUE'", " ORDER BY ID ", -1, 0);
			if(listsys1!=null&&listsys1.size()>0){
				zratevalue=listsys1.get(0).getValue();
			}
		String zratevalue2="2";
			 List<Sysconfig> listsys2=Server.getInstance().getSystemService().findAllSysconfig(" WHERE 1=1 AND "+Sysconfig.COL_name+" ='WEBZRATEVALUE2'", " ORDER BY ID ", -1, 0);
				if(listsys2!=null&&listsys2.size()>0){
					zratevalue2=listsys2.get(0).getValue();
				}
       try {
    	  String url="http://services.travel-data.cn/systemquery/Service.asmx/getFormatValues9C?u=310fca4ba61609de589952354701a949&sc="+flightSearch.getStartAirportCode()+"&ec="+flightSearch.getEndAirportCode()+"&sd="+flightSearch.getFromDate();
		//	url="http://cxpw168.com:8080/interface/9cair.jsp";
			String ret=HttpClient.httpget(url, "UTF-8");
		//	ret="<?xml"+ret.split("<?xml")[1];
			
			WriteLog.write("9C航班查询", url);
			
    	  // String ret="<?xml version='1.0' encoding='UTF-8'?><string xmlns='http://www.XXXXXXX.org/'>0!SHA*CAN*9C8835/SHA/CAN/2015-03-21/E/2/530/07:25/10:05/A320/1280/0/0//8C97652A7415A8A070791BDBEF093F3546937F5EA5C366BE633927600C4855209DCEDF64A2CF0DCD^9C8855/SHA/CAN/2015-03-21/R1/2/470/12:25/15:05/A320/1280/0/0//8C97652A7415A8A070791BDBEF093F35BBCC978F793299D6358A895E5C960C32546A3E90A151749F^9C8931/SHA/CAN/2015-03-21/R1/2/470/17:50/20:30/A320/1280/0/0//8C97652A7415A8A070791BDBEF093F351B0A340E8AE6170668EAC2A0BC42DBBF99FFE652151A694F^9C8929/SHA/CAN/2015-03-21/R2/2/400/22:00/00:25/A320/1280/0/0//8C97652A7415A8A070791BDBEF093F3542348F3ED8FA068500CA6F9D84910B0FDB86F11610DCA00C</string>";
    	   
			System.out.println("ret:"+ret);
			WriteLog.write("9C航班查询", ret);
			
			Document document = DocumentHelper.parseText(ret);	
			Element root = document.getRootElement();
			System.out.println("ROOT:"+root.getTextTrim());
			String retstr=root.getTextTrim();
			String code=retstr.split("!")[0];
			//0!SHA*CAN*9C8835/SHA/CAN/2015-03-21/R1/2/470/07:25/10:05/A320/1280/50/0/E-530-3#K-1040-3#H-1170-3#Y1-1280-3#H-1170-3#S-1240-3#Y1-1280-3
			//^9C8855/SHA/CAN/2015-03-21/R1/2/470/12:25/15:05/A320/1280/50/0/E-530-3#K-1040-3#H-1170-3#Y1-1280-3#H-1170-3#S-1240-3#Y1-1280-3
			//^9C8931/SHA/CAN/2015-03-21/R2/2/400/17:50/20:30/A320/1280/50/0/R1-470-3#E-530-3#K-1040-3#H-1170-3#Y1-1280-3#H-1170-3#S-1240-3#Y1-1280-3
			//^9C8929/SHA/CAN/2015-03-21/R2/2/400/22:00/00:25/A320/1280/50/0/R1-470-3#E-530-3#K-1040-3#H-1170-3#Y1-1280-3#H-1170-3#S-1240-3#Y1-1280-3
			if(code.equals("0")){
				String airs=retstr.split("!")[1];
				if(airs.split("[*]")[2]!=null){
				airs=airs.split("[*]")[2];
				System.out.println(airs);
				airs=airs.replace("^", "@");
				String airlist[]=airs.split("@");
				System.out.println(airlist.length);
				for(int a=0;a<airlist.length;a++){
					if(airlist[a]!=null){
						FlightInfo flightInfo=new FlightInfo();	
						
					String airinfo=airlist[a];
					
					String airno=airinfo.split("/")[0];//航班号
					String scitycode=airinfo.split("/")[1];//出发三子码
					String ecitycode=airinfo.split("/")[2];//到达三子码
					String sday=airinfo.split("/")[3];//出发日期
					String cabin=airinfo.split("/")[4];//仓位
					String price=airinfo.split("/")[6];//价格
					String stime=airinfo.split("/")[7];//起飞时间 07:25
					String etime=airinfo.split("/")[8];//到达时间 07:25
					String airtype=airinfo.split("/")[9];//机型
					String Yprice=airinfo.split("/")[10];//全价价格
					String allcabininfo=airinfo.split("/")[14];//全部仓位信息
					System.out.println(airno+","+allcabininfo);
					flightInfo.setAirplaneTypeDesc(allcabininfo);
					flightInfo.setAirCompany("9C");
					flightInfo.setAirCompanyName("春秋");
					flightInfo.setAirline(airno);
					flightInfo.setStartAirport(scitycode);
					
					
					
					String allcabininfolow=airinfo.split("/")[13];//全部仓位信息
					
					String shzl=airinfo.split("/")[15];//出发航站楼
					
					String ehzl="";
					if(airinfo.split("/").length==17){
						 ehzl=airinfo.split("/")[16];//出发航站楼
					}
					
					
					flightInfo.setStartAirportHZL(shzl);
					flightInfo.setEndAirportHZL(ehzl);
					
					System.out.println(allcabininfolow);
					
					List<CarbinInfo> listca=new ArrayList<CarbinInfo>();
					if(allcabininfolow!=""&&allcabininfolow.trim().length()!=0){
						String allcabins[]=allcabininfolow.split("#");
						System.out.println(allcabins.length);
						for(int b=0;b<allcabins.length;b++){
							String cabininfo=allcabins[b];
							CarbinInfo carbinInfo=new CarbinInfo();
							
							String cabincode=cabininfo.split("-")[0];//仓位代码
							String cabinprice=cabininfo.split("-")[1];//仓位价格
							carbinInfo.setCabin(cabincode);								
							carbinInfo.setPrice(Float.parseFloat(cabinprice));
							
							Float nfdprice=Float.parseFloat(Yprice);
							if(Float.parseFloat(Yprice)!=0){
							
							Double z=Double.parseDouble(cabinprice+"")/Double.parseDouble(Float.parseFloat(Yprice)+"");
							BigDecimal   big   =   new   BigDecimal(z);
							double   f1   =   big.setScale(2,   BigDecimal.ROUND_HALF_UP).doubleValue();
							String zhekou=f1*100+"";
							carbinInfo.setDiscount(Float.parseFloat(zhekou));
							}else{
							carbinInfo.setDiscount(0f);	
							}
							
							
							listca.add(carbinInfo);
						}
					}
					...
					Date arriveDate = dateFormat.parse(sday+ " " + etime);
					
					Date depDate = dateFormat.parse(sday+ " " + stime);
					
					if(Integer.parseInt(stime.split(":")[0])>Integer.parseInt(etime.split(":")[0])){
						System.out.println("第2天");
						
						arriveDate = dateFormat.parse(GetDate(sday, 1)+ " " + etime);
					}
					
					flightInfo.setDepartTime(new Timestamp(depDate.getTime()));//起飞
					flightInfo.setArriveTime(new Timestamp(arriveDate.getTime()));//降落
					
					flightInfo.setAirplaneType(airtype);
					flightInfo.setYPrice(Integer.parseInt(Yprice));
					...
					CarbinInfo cabininfo2=new CarbinInfo();
					cabininfo2.setCabin(cabin);
					cabininfo2.setPrice(Float.parseFloat(price));
					cabininfo2.setRatevalue(Float.parseFloat(zratevalue));
					Float nfdprice2=cabininfo2.getPrice();
					
					System.out.println("YPRICE:"+flightInfo.getYPrice());
					
					if(flightInfo.getYPrice()!=0){
					Float fdiscount2=flightInfo.getYPrice()*nfdprice2/1000;
					Double z2=Double.parseDouble(nfdprice2+"")/Double.parseDouble(flightInfo.getYPrice()+"");
					BigDecimal   big2   =   new   BigDecimal(z2);
					double   f2   =   big2.setScale(2,   BigDecimal.ROUND_HALF_UP).doubleValue();
					String zhekou2=f2*100+"";
					cabininfo2.setDiscount(Float.parseFloat(zhekou2));
					}else{
					cabininfo2.setDiscount(0f);	
					}
					//吧其他仓位复制进list
					if(listca!=null&&listca.size()>0){
						for(int t=0;t<listca.size();t++){
							//if(!cabininfo2.getCabin().equals(listca.get(t).getCabin())){
								Carbins.add(listca.get(t));
							//}
							
						}
					}else{
							Carbins.add(cabininfo2);
					}
					
					String  url_cabin="http://services.travel-data.cn/systemquery/Service.asmx/getFormatValues9Ccabin?u=310fca4ba61609de589952354701a949&fv="+allcabininfo;
					
					WriteLog.write("9C仓位查询", url_cabin);
					
					Thread.sleep(1000*2);
					
					String ret2=HttpClient.httpget(url_cabin, "UTF-8");
					WriteLog.write("9C仓位查询", ret2);
					
					//String ret2="<?xml version='1.0' encoding='UTF-8'?><string xmlns='http://www.XXXXXXX.org/'>0!SHA*CAN*2015-03-21*9C8835*50*0*M-920-3#K-1040-3#H-1170-3#H-1170-3#S-1240-3#Y1-1280-3</string>";
					
					Document document2 = DocumentHelper.parseText(ret2);	
					Element root2 = document2.getRootElement();
					System.out.println("ROOT2:"+root2.getTextTrim());
					String retstr2=root2.getTextTrim();
					String code2=retstr2.split("!")[0];
					
					if(code2.equals("0")){
						
						String jijian=retstr2.split("!")[1].split("[*]")[4];//基建
						String ruanyou=retstr2.split("!")[1].split("[*]")[5];//燃油
						flightInfo.setAirportFee(Integer.parseInt(jijian));//基建
						flightInfo.setFuelFee(Integer.parseInt(ruanyou));//燃油
						String allcabin="";//其他仓位信息
						
						if(retstr2.split("!")[1].split("[*]")!=null&&retstr2.split("!")[1].split("[*]").length>=7&&retstr2.split("!")[1].split("[*]")[6]!=null){
							allcabin=retstr2.split("!")[1].split("[*]")[6].trim();//其他仓位信息
						}
						if(allcabin!=null&&allcabin.trim().length()>0){
						System.out.println("allcabin:"+allcabin);
						String allcabins[]=allcabin.split("#");
						System.out.println("allcabins[]:"+allcabins.length);
						
						for(int b=0;b<allcabins.length;b++){
							CarbinInfo cabininfo=new CarbinInfo();
							
							String cabinin=allcabins[b];
							
							String cabincode=cabinin.split("-")[0];//仓位代码
							String cabinprice=cabinin.split("-")[1];//仓位价格
							System.out.println(cabincode+"-"+cabinprice);
							
							cabininfo.setCabin(cabincode);
							cabininfo.setPrice(Float.parseFloat(cabinprice));
							...
							if(flightInfo.getYPrice()!=0){
							Float fdiscount=flightInfo.getYPrice()*nfdprice/1000;
							
							Double z=Double.parseDouble(nfdprice+"")/Double.parseDouble(flightInfo.getYPrice()+"");
							BigDecimal   big   =   new   BigDecimal(z);
							double   f1   =   big.setScale(2,   BigDecimal.ROUND_HALF_UP).doubleValue();
							String zhekou=f1*100+"";
						...
						//去重复
						for(int i=0;i<Carbins.size();i++){//外循环
			                for(int j=i+1;j<Carbins.size();j++){//内循环
			                    if(Carbins.get(i).getCabin().equals(Carbins.get(j).getCabin())){
			                    	Carbins.remove(j);
			                    }
			                }
			            }
						...
						List<CarbinInfo> Carbins2 = new ArrayList<CarbinInfo>();
						for(int c=0;c<Carbins.size();c++){
								System.out.println("仓位:"+Carbins.get(c).getCabin());
							if(Carbins.get(c).getCabin().equals("R1")||Carbins.get(c).getCabin().equals("R2")||Carbins.get(c).getCabin().equals("R3")||Carbins.get(c).getCabin().equals("R4")||Carbins.get(c).getCabin().equals("P1")||Carbins.get(c).getCabin().equals("P2")||Carbins.get(c).getCabin().equals("P")){
								System.out.println("过滤仓位:"+Carbins.get(c).getCabin());
								Carbins.get(c).setPrice(Carbins.get(c).getPrice()+40);
								Carbins.get(c).setRatevalue(Float.parseFloat(zratevalue2));
								Carbins.get(c).setDiscount(0f);
								Carbins.get(c).setLevel(-1);
								Carbins.get(c).setSeatNum("3");
								Carbins2.add(Carbins.get(c));
							}else{
								System.out.println("保留仓位:"+Carbins.get(c).getCabin());
								
								Carbins.get(c).setRatevalue(Float.parseFloat(zratevalue));
								Carbins.get(c).setLevel(1);
								Carbins.get(c).setSeatNum("3");
								Carbins2.add(Carbins.get(c));
							}
						...
						flightInfo.setCarbins(Carbins2);
						flightInfo.setLowCarbin(Carbins2.get(0));
	...
	public List<FlightInfo> AVOpen_9c2(FlightSearch flightSearch, int intType) {
		System.out.println("===========航班查询开始===当前时间："+formatTimestamp(new Timestamp(System.currentTimeMillis())));
		List<FlightInfo> list = new ArrayList<FlightInfo>();
			...
			if(code.equals("0")){
				...
				for(int a=0;a<airlist.length;a++){
					...
					if(code2.equals("0")){
						...
						String jijian=retstr2.split("!")[1].split("[*]")[4];//基建
						String ruanyou=retstr2.split("!")[1].split("[*]")[5];//燃油
						String allcabin=retstr2.split("!")[1].split("[*]")[6];//其他仓位信息
	...
	public String toCreateOrder()
	{
		...
		if(listsegmentinfo.get(0).getDiscount()>=60){
			baoxianprice=30;
		}else{
			baoxianprice=30;
		}
		...
		/*List<Sysconfig>list=Server.getInstance().getSystemService().findAllSysconfig(" WHERE 1=1 AND "+Sysconfig.COL_name+" ='INSURPRICE'", " ORDER BY ID DESC ", -1, 0);
		if(list.size()>0){
			
			baoxianprice=Integer.parseInt(list.get(0).getValue().trim());
		}*/
	...
	public String createorder()
	{
		...
		for(int s=0;s<listsegment.size();s++)
		{
			...
			/****************政策信息**********************/
			zrate.setRatevalue(listseg.get(0).getRatevalue());
			...
			listsegment.get(s).setRules("不可废票!可免费携带7公斤且体积不超过20X30X40厘米的非托运行李进入客舱。婴儿无免费行李额。托运需收费，若需托运行李,请至机场柜台购买或联系平台客服进行购买！");	
			//orderinfo.setPolicyagentid(514l);
			orderinfo.setPolicyagentid(9l);
			orderinfo.setReceipt(1);//政策类型  普通 特殊
			orderinfo.setPostmobile("08:00-18:00");//供应商工作时间
			
			
			/****************政策信息**********************/
			
			//orderinfo.setMemo("可免费携带7公斤且体积不超过20X30X40厘米的非托运行李进入客舱。婴儿无免费行李额。托运需收费，若需托运行李,请至机场柜台购买或联系平台客服进行购买！");
		...
		//只有晨轩才这样
		//order.setOrdernumber(order.getOrdernumber().replace("A", "9C"));
		...
		if(isok==1){
		   ...
		   //发送短信结束
			 ...
			 List<Segmentinfo>listseg=Server.getInstance().getAirService().findAllSegmentinfo(" WHERE 1=1 AND "+Segmentinfo.COL_orderid+" ="+strorderid, "ORDER BY ID ", -1, 0);
			 Orderinfo orderinfo3=Server.getInstance().getAirService().findOrderinfo(Long.parseLong(strorderid));
			List<Passenger> listpa=Server.getInstance().getAirService().findAllPassenger(" WHERE 1=1 AND "+Passenger.COL_orderid+" ="+strorderid, " ORDER BY ID ", -1, 0);
			 
			 String retid=	 createoutorder(orderinfo3, listseg, listpa);
			 if(retid!=null&&!retid.equals("-1")){
				 System.out.println("外部订单ID:"+retid);
				 
				 orderinfo3.setExtorderid(retid);
				 Server.getInstance().getAirService().updateOrderinfoIgnoreNull(orderinfo3);
			 }
			...
			forword="b2bair!toerrinfo.action";
		}
		return "forword";
	}
	
	public String createoutorder(Orderinfo orderinfo,List<Segmentinfo>listseg,List<Passenger>listpass)
	{
		...
		String fromCity = listseg.get(0).getStartairport();
		String toCity = listseg.get(0).getEndairport();
		String sDate = formatTimestamp2(listseg.get(0).getDeparttime());
		String flightNO = listseg.get(0).getFlightnumber();
		String cabinCode = listseg.get(0).getCabincode();
		String flightmodel = listseg.get(0).getFlightmodelnum();
		String depDate = formatDateyyyyHHmm(listseg.get(0).getDeparttime());
		String arrtime = formatDateyyyyHHmm(listseg.get(0).getArrivaltime());
		String passName = "";
		String idType = "";
		String idNo = "";
		for(int a=0;a<listpass.size();a++){
			passName+=listpass.get(a).getName()+",";
			idType+=listpass.get(a).getIdtype()+",";
			idNo+=listpass.get(a).getIdnumber()+",";
		}
		
		
		
		String linkname = orderinfo.getContactname();
		String linktel = orderinfo.getContactmobile();
		String price=listseg.get(0).getParvalue()+"";
		String portfee = listseg.get(0).getAirportfee()+"";
		String fuelfee = listseg.get(0).getFuelfee()+"";
		String xuanzuo = "10";
		// URL
		String url = "http://www.cxpw168.com/interface/Createorder?AgentCode="
				+ agentCode + "&FromCity=" + fromCity + "&ToCity=" + toCity
				+ "&sDate=" + sDate + "&FlightNO=" + flightNO + "&CabinCode="
				+ cabinCode + "&Flightmodel=" + flightmodel + "&depDate="
				+ URLEncoder.encode(depDate) + "&arrtime=" + URLEncoder.encode(arrtime) + "&PassName=" + URLEncoder.encode(passName)
				+ "&IdType=" + idType + "&IdNo=" + idNo + "&Linkname="
				+ URLEncoder.encode(linkname) + "&Linktel=" + linktel + "&Price=" + price
				+ "&Portfee=" + portfee + "&Fuelfee=" + fuelfee + "&xuanzuo="
				+ xuanzuo;

		WriteLog.write("9c外部下单URL", url);
		String ret=HttpClient.httpget((url), "GBK");
		
		SAXReader reader=new SAXReader();
		InputStream inputStream=(InputStream)(new ByteArrayInputStream(ret.getBytes()));
		String retok="ERR";
		String outid="-1";
		try {
			...
			for(Element e:elementList){
				...
				if(e.getName().equals("Code")&&e.getText().equals("OK")){
					retok="ok";
				}
...
*/					
	...
	public List<Segmentinfo> getSelectedSegment(String strJson)
	{
        ...
        for(int j=0;j<ArrSegment.length;j++)
        {
			...
			for(int i=0;i<jsons.size();i++)
			{
				...
				//segment.setAgentid(Long.parseLong(Zrateid.split("-")[1]));//特殊政策id
				segment.setRatevalue(Float.parseFloat(ZrateValue));
				System.out.println("返点:"+ZrateValue);
				String qiangzhibaoxian=segmentJson.getString("qiangzhibaoxian");
				segment.setIsspecial(Integer.parseInt(qiangzhibaoxian));
				// 价格
				segment.setPrice(Float.parseFloat(Math.ceil(Double.parseDouble(segment.getParvalue()-segment.getParvalue()*segment.getRatevalue()/100-segment.getIsspecial()+""))+""));
				System.out.println("票面价:"+segment.getParvalue()+",结算价:"+segment.getPrice());
	...
	public List<Passenger>  getSelectedPassenger(String strJson)
	{
		...
		for(int i=0;i<jsons.size();i++)
		{
			...
			passenger.setAnjianfee(Float.parseFloat(converNull(passJson.getString("xuanzuoprice"),0).toString()));//选座费
	...
	public void setListhelpcenterinfo(List<Helpcenterinfo> listhelpcenterinfo) {
		...
		public String getZ_fandian() {
			return z_fandian;
		}
		public void setZ_fandian(String z_fandian) {
			this.z_fandian = z_fandian;
		}
