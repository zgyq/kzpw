 ...
 * 
 */
public class HoteluserbookAction extends B2b2cbackAction {
	//预订酒店
...
//周几信息
		...
		if(seachHotelBean!=null&&seachHotelBean.getListRoomtype()!=null){
			...
			for(int a=0;a<listroom.size();a++){
				...
				//String shourijia=listRoomRatePlan.get(0).getAverageRate();
				//String shourijia=listRoomRatePlan.get(0).getAverageRate();
				String shourijia=listRoomRatePlan.get(0).getListNightlyRate().get(0).getMember();
	...
	public void findHotelRoomPrice_ELONG() {
		...
		String sub="<table width='100%' border='0' cellspacing='0' cellpadding='0'  style='line-height: 35px; text-align: center; border-collapse: collapse;'>";
		...
		if(seachHotelBean!=null&&seachHotelBean.getListRoomtype()!=null){
			...
			List<PrepayRule>ListPrepayRule=seachHotelBean.getListPrepayRule();//担保规则
			//List<Hotelprice>listroomprice=seachHotelBean.getListHotelprice();
			sub+="<tr bgcolor='#537cb4' class='hui14'>";
			sub+="<td width='25%' class='box_r'>客房类型</td>";
			sub+="<td width='10%' class='box_r'>门市价</td>";
			sub+="<td width='10%' class='box_r'>首日价</td>";
			sub+="<td width='15%' class='box_r'>首日返利</td>";
			sub+="<td width='15%' class='box_r' >床型</td>";
			sub+="<td width='10%' class='box_r'>早餐</td>";
			sub+="<td width='10%' class='box_r'>宽带</td>";
			sub+="<td class='box_r'>操作</td>";
			sub+="</tr>";
			...
			for(int a=0;a<listroom.size();a++){
				...
				str.append("<td width='25%' class='box_r'>"+listroom.get(a).getName()+"</td>");
				str.append("<td width='10%' class='box_r' style='text-decoration: line-through'>￥"+menshijia+"</td>");
				str.append("<td width='10%' class='box_r'>￥"+shourijia+"</td>");
				...
				try
				 ...
				 str.append("<td width='15%' class='box_r'>￥"+roomZrateValue+"</td>");
				str.append("<td width='15%' class='box_r'>"+BED+"</td>");
				str.append("<td width='10%' class='box_r'>"+Breakfast+"</td>");
				str.append("<td width='10%' class='box_r'>"+Wideband+"</td>");
				System.out.println(listroom.get(a).getListRatePlan().get(0).getStatus());
				if(listroom.get(a).getListRatePlan().get(0).getStatus().equals("true")){
					str.append("<td class='box_r'><input type='button'  class='button_yuding' value='预订' onclick='yud("+listroom.get(a).getId()+","+hot.getId()+");'  /></td>");
				}else{
			...
			str.append("<tr>");
			str.append("<td align='right' id='"+s_hotelid+"_td1' colspan='8' style='height: 16px; line-height: 16px;'>");
			str.append("<span  style='cursor: pointer;' onclick='showrooom("+s_hotelid+","+listroom.size()+");' >展开房型▼</span>");
			str.append("</td>");
			str.append("<td align='right' id='"+s_hotelid+"_td2' colspan='8' style='height: 16px; line-height: 16px; display: none'>");
			str.append("<span  style='cursor: pointer;' onclick='hidrooom("+s_hotelid+","+listroom.size()+");' >收起房型▼</span>");
			str.append("</td>");
			str.append("</tr>");
			if(ListBookingRules!=null){
			str.append("<tr style='color: red'>");
			str.append("<td colspan='8' align='left'>");
			if(ListBookingRules!=null){
				str.append("预定须知:");
				for(int b=0;b<ListBookingRules.size();b++){
					str.append(ListBookingRules.get(b).getDescription()+"</br>");
		...
		//System.out.println(sub+str.toString());
		
		Writer writer;
		try {
			writer = response.getWriter();
			writer.write(sub+str.toString()+"</table>");
			writer.flush();
			writer.close();
		} catch (IOException e) {
			// TODO Auto-generated catch block
...
//
	...
	if(seachHotelBean!=null&&seachHotelBean.getListRoomtype()!=null){
		...
		priceUsertype=listRatePlan.get(0).getCustomerType();
		System.out.println("priceUsertype:"+priceUsertype);
...
pr=	GetHotelPriceValueByStar(hotel.getStar().toString(), Float.parseFloat(hotelorder.getPrice()));
	...
	hotelorder.setProfits(Double.parseDouble(hotelorder.getPrice())*Double.parseDouble(hotelZrateValue)/100);
	hotelorder.setPricecodeid(pricecode);//价格计划ID
	hotelorder.setPricecodename(priceUsertype);//客人类型
...
//添加信用卡信息
	...
	if(cardcvv!=null&&!cardcvv.equals("")&&cardcvv.trim().length()>0){
	creditcard.setCreditcheckcode(cardcvv);//CVV
	}
	creditcard.setCustomeruserid(getLoginUser().getId());
...
if(wcode!=null&&wcode.indexOf("-1")==-1){
...
//	hotelorder.setProfits(Double.parseDouble(wcode.split("@")[1].trim()));//订单总利润
	...
	hotelorder.setWaicode(wcode);
	hotelorder.setState(88);
	System.out.println("elong酒店下单失败:"+wcode);
	
	errinfo=wcode.split("@")[1].split("[|]")[0];
...
Server.getInstance().getHotelService().updateHotelorderIgnoreNull(hotelorder);
	...
	*/
	...
	if(hotelorder.getState()==1){
		...
		forword ="hoteluserbook!toshibai.action?errinfo="+errinfo;
...
public String GetElongErrByCode(String code){
	String ret="";
...
//
	...
	//	Hotel hot = Server.getInstance().getHotelService().findHotel(hid);
		...
		if(listImages.size() > 0) {
			if(listImages.get(0).getPath()==null||listImages.get(0).getPath().trim().length()==0){
					if(listImages.get(0).getDescription()==null||listImages.get(0).getDescription().length()==0){
						return "images/NoImage.gif" ;
					}else{
							if(!listImages.get(0).getDescription().substring(0, 4).equals("http")){
							return "http://www.elongstatic.com/imageapp/hotels/hotelimages"+listImages.get(0).getDescription().split("@")[0] ;
							}else{
								
							return listImages.get(0).getDescription().split("@")[0] ;	
							}
					}
	 ...
	 * @return  图片路径
	 */
	public String getImageSRcinfo(String path) {
	//	Hotel hot = Server.getInstance().getHotelService().findHotel(hid);
		
		
		
			if(path.indexOf("http")==-1){
			return "http://www.elongstatic.com/imageapp/hotels/hotelimages"+path;
			}else{
