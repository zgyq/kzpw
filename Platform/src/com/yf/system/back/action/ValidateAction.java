...
public class ValidateAction extends B2b2cbackAction {
	...
	public void SearchNumInfo() throws IOException{
			...
			if(!Server.getInstance().getAirService().findAllXcdnoinfo("where 1 = 1 and "+Xcdnoinfo.COL_xcdinfo+" = '"+validateValue+"'","",-1,0).isEmpty()){
...
//申请用
public void validateusername2() throws Exception {
	ActionContext ac = ActionContext.getContext();
	HttpServletRequest request = (HttpServletRequest) ac
	.get(ServletActionContext.HTTP_REQUEST);
	String validateId = request.getParameter("validateId");
...
public void validateAgentName2() throws Exception {
	ActionContext ac = ActionContext.getContext();
	HttpServletRequest request = (HttpServletRequest) ac
	.get(ServletActionContext.HTTP_REQUEST);
	
	String validateId =  new String(request.getParameter("s_name").getBytes("ISO8859-1"), "utf-8");
	
	System.out.println(validateId);
	PrintWriter out = ServletActionContext.getResponse().getWriter();
	if(!Server.getInstance().getMemberService().findAllCustomeragent("where 1 = 1 and "+Customeragent.COL_agentcompanyname+" = '"+validateId+"'","",-1,0).isEmpty()){
		out.print("false");
		//验证失败写false 成功返回true
	}else{
		out.print("true");
