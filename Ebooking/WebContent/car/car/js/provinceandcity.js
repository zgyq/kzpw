...
cityArray[19] = new Array("广东省","广州|杭州|珠海|汕头|东莞|中山|佛山|韶关|江门|湛江|茂名|肇庆|惠州|梅州|汕尾|河源|阳江|清远|潮州|揭阳|云浮"); 
...
cityArray[34] = new Array("其他","火星|太阳|月球|冥王星|那美克星|黑洞"); 

        function getCity(currProvince)
        {
         currProvince=$("#selProvince").val();
...
shengArray = new Array("北京市","上海市","天津市","重庆市","河北省","山西省","内蒙古自治区","辽宁省","吉林省","黑龙江省","江苏省","浙江省","安徽省","福建省","江西省","山东省","河南省","湖北省","湖南省","广东省","广西壮族自治区","海南省","四川省","西藏自治区","山西省","甘肃省","宁夏回族自治区","青海省","新疆维吾尔族自治区","香港特别行政区","澳门特别行政区","台湾省","其他");
	...
	//alert("111");
	var s=0;
	//alert(shengArray.length);
		for(s=0;s<shengArray.length;s++){
			//填充省分下拉列表框
			//alert(shengArray[s]);
			
			//document.all.selProvince.options[document.all.selProvince.length]=new Option(shengArray[s]);
			
			shenghtml+="<option value='"+shengArray[s]+"'>"+shengArray[s]+"</option>";
			
		}
		$("#selProvince").html(shenghtml);
	}
