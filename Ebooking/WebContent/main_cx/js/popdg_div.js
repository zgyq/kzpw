...
var csms = new Array;
csms[0] = ["北京|PEK|B", "上海|SHA|S", "广州|CAN|G", "深圳|SZX|S", "成都|CTU|C", "杭州|HGH|H", "武汉|WUH|W", "西安|XIY|X", "重庆|CKG|C", "青岛|TAO|Q", "长沙|CSX|C", "南京|NKG|N", "厦门|XMN|X", "昆明|KMG|K", "大连|DLC|D", "天津|TSN|T", "郑州|CGO|Z", "三亚|SYX|S", "济南|TNA|J", "福州|FOC|F"];
...
csms[4]=["乌兰浩特|HLH|S","台州|HYN|T","且末|IQM|Q","庆阳|IQN|Q","黔江|JIQ|S","衢州|JUZ|Q","齐齐哈尔|NDG|Q","上海|SHA|S","攀枝花|PZI|P","日喀则|RKZ|R","沈阳|SHE|S","秦皇岛|SHP|Q","沙市|SHS|S","石家庄|SJW|S","普洱(思茅)|SYM|S","三亚|SYX|S","深圳|SZX|S","青岛|TAO|Q","塔城|TCG|T","铜仁|TEN|T","通辽|TGO|T","天水|THQ|T","天津|TSN|T","唐山|TVS|T","太原|TYN|T","乌鲁木齐|URC|W","潍坊|WEF|W","威海|WEH|W","文山|WNH|W","温州|WNZ|W","乌海|WUA|W","武汉|WUH|W","武夷山|WUS|W","无锡|WUX|W","梧州|WUZ|W","万州|WXN|W","上海(虹桥)|SHA|S","上海(浦东)|PVG|S","泉州晋江|JJN|Q","汕头|SWA|S","鄯善|SXJ|S","韶关|SHG|S"];
csms[5]=["兴义|ACX|X","郑州|CGO|Z","张家界||Z","延安|ENY|Y","舟山|HSN|Z","银川|INC|Y","西双版纳|JHG|X","伊春|LDS|Y","永州|LLF|Y","榆林|UYN|Y","襄樊|XFN|X","西昌|XIC|X","锡林浩特|XIL|X","西安|SIA|X","厦门|XMN|X","西宁|XNN|X","徐州|XUZ|X","宜宾|YBP|Y","运城|YCU|Y","宜昌|YIH|Y","伊宁|YIN|Y","义乌|YIW|Y","延吉|YNJ|Y","烟台|YNT|Y","盐城|YNZ|Y","张掖|YZY|Z","昭通|ZAT|Z","中山|ZSW|Z","中卫|ZHY|Z","湛江|ZHA|Z","珠海|ZUH|Z","遵义|ZYI|Z","香格里拉|DIG|X","张家界|DYG|Z"];
var ywzm_en = new Array("rm","A-F","G-J","K-N","P-W","X-Z"); 
var ywzm = new Array("热门","A-F","G-J","K-N","P-W","X-Z"); 
...
function popUp(abc,code){
    ...
    popup_int3=0;
    var tab;
    tab = '<div id="city" class="box_nei">';
    tab += '<div class="piny" ><span class="f">热门城市（可直接输入城市名称或拼音）</span></div>';
	tab+='<div class="c"></div><div id="divzhedang1"></div><div class="hr"></div>';
    tab+= '<div id="city-box" class="nav-a_z">';
    tab+= '<ul>';
    for(var ywzm_i=0;ywzm_i<6;ywzm_i++){
    	if(!ywzm[ywzm_i])break;
    	tab+= '<li id="syy_'+ywzm_en[ywzm_i]+'" class="f city-nav" onclick="f1(id,'+"'"+ywzm_d[ywzm_i]+"'"+')"><span style="cursor: pointer;" href="#">'+ywzm[ywzm_i]+'</span></li>';
    }
	 tab+= '</ul>';
	 tab+= '</div>';
	 tab+='<div style="clear:both;"></div><div class="hr1"></div>'
	 for(var cs=0;cs<6;cs++){
		 ...
		 for(var csm=0;csm<csms[cs].length;csm++ ){
			 ...
			 tab+= '<li  class="f cityon"><a  id='+id1+' href="javascript:void(0);">'+value1+'</a></li>';	
			 }
