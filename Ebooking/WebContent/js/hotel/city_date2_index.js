...
//生成并显示出城市下拉菜单
function GetCityList_spot(fctThisObj){
	...
	for(var i=0;i<varCityDataSplit.length-1;i++){
		...
		if(varCityDataSplitI.toUpperCase().indexOf(varObjValue.toUpperCase())>=0 || varObjValue=="" || i==varObjValue){ // || varCityDataSplitI.toLowerCase().indexOf(varObjValue.toLowerCase())>=0
			...
			if(varThisPageI>varPageId*varPageRCount && varThisPageI<=(varPageId+1)*varPageRCount){
				...
				varHtmlStr+="<a href='javascript:;' onclick='WriteCity_spot(\""+varCityDataSplitI.split("&")[1]+"|"+varCityDataSplitISplit[0]+"\")' id='menuA"+varThisPageI+"' title='"+varCityDataSplitI.split("%")[0].split("#")[1] +'|'+varCityDataSplitISplit[0] +"'><span id='menuA"+varThisPageI+"'9'>"+varCityDataSplitISplit[0]+"&nbsp;&nbsp;</span>"+varCityDataSplitI.split("%")[0].split("#")[1]+"</a>";
			}
		}
	}
...
//选择某城市
function WriteCity_spot(fctI){
	varMenuValue=fctI.split("|");
	//$("#city_hotel_hide").val(varMenuValue[0]);
	document.getElementById("city_hotel_hide").value=varMenuValue[0];
	//$("#txthotelcity").val(varMenuValue[1]);
	document.getElementById("txthotelcity").value=varMenuValue[1];
}
//隐藏城市列表
function _Hidden(e){
	e=e?e:event;
