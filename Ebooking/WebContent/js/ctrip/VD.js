...
(function(){
    var VacationSearch=function(){
        var _iptStartCity=document.getElementById("txtVacationStartCity"),
            _iptDestCity=document.getElementById("txtVacationDestCity"),
            _btnSearch=document.getElementById("iptVacationSearchBtn"),
            _panelStartCity=document.getElementById("divVacationStartCity"),
            _body=document.getElementsByTagName("BODY")[0],
            _hiddenVal=document.getElementById("solution").value.split('|')[1],
            _encode = (_hiddenVal==="BIG5" || _hiddenVal==="HK") ?  "big5":"gb2312",
            _startCity=2,
            _refVacation,
            initStartCity=function(){
                var _cookiest = Cmd.Cookie.GetCookie("StartCity_Pkg");
                if(_cookiest!=null){
                    var  _cookieCity=_cookiest.split("=");
                    if(_cookieCity.length>1){
                        var _temp =_cookieCity[1],
                            _arrs=$("#divVacationStartCity").find("a[data-id]");
                        for(var i= 0,l=_arrs.length;i<l;i++){
                            if(_temp==_arrs[i].getAttribute("data-id")){
                                _startCity=_temp;
                                _iptStartCity.value=_arrs[i].innerHTML;
                                break;
                            }
                        }
                    }
            ...
            tipLoc=function(pAim,pTip){
                var _pos = UI.Dom.position(pAim);
                pTip.style.top=_pos.y+pAim.clientHeight+"px";
                pTip.style.left=_pos.x+"px";
            },
            ...
            initCity=function(){
                var me =this;
                Core.bind(_iptStartCity,"click",function(event){
                    _panelStartCity.style.display="block";
                    tipLoc(_iptStartCity,_panelStartCity);
                    event.stopPropagation();
                });
                Core.bind(_panelStartCity,"click",function(event){
                    var _tar =event.target;
                    if(_tar.getAttribute("data-id")){
                        _panelStartCity.style.display="none";
                        VacationConfig.StartCity=_startCity=_tar.getAttribute("data-id");
                        _iptStartCity.value=_tar.innerHTML;
                        Core.Storage.getSession("VacationAddressData"+_startCity,VacationConfig.AddressDataUrl,"startCity="+_startCity,function(data){
                            _refVacation.changeSource(data);
                        },true,_encode);
                    }
                });
                Core.bind(_body, "click", function (event) {
                    _panelStartCity.style.display="none";
                });
            ...
            initSearch=function(){
                var me =this;
                defaultTip(_iptDestCity);
                Core.Storage.getSession("VacationAddressData"+_startCity,VacationConfig.AddressDataUrl,"startCity="+_startCity,function(data){
                    _refVacation= new SearchAddress($(_iptDestCity),"VacationSearch",data,null,function(){
                        var _val = _iptDestCity.value.replace(/\s|\t|\n|\r/g,"");
                        if(_val !=="" && _val !== _iptDestCity.getAttribute("data-default")){
                            vacationSearch.call(me,_iptDestCity);
                            ins.method("hide");
                        } else{
                            ins.method("show",{
                                $obj:$(_iptDestCity),
                ...
                var ins = $("#vacationTab").regMod('validate',"1.1");
                Core.bind(_btnSearch,"click",function(){
                    var _val = _iptDestCity.value.replace(/\s|\t|\n|\r/g,"");
                    if(_val !=="" && _val !== _iptDestCity.getAttribute("data-default")){
                        vacationSearch.call(me,_iptDestCity);
                        ins.method("hide");
            ...
            init=function(){
                var me = this;
                initStartCity.call(me);
                initCity.call(me);
                initSearch.call(me);
                if(_encode==="big5"){
                    var _a = $("#vacationTab a");
